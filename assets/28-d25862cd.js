const n={id:"28",title:"28 | WebComponent：像搭积木一样构建Web应用",content:`<h1 id="28--webcomponent：像搭积木一样构建web应用">28 | WebComponent：像搭积木一样构建Web应用</h1>
...
<h2 id="阻碍前端组件化的因素">阻碍前端组件化的因素</h2>
...
<p>我们在<a href="/markdown-reader/guide/23">《23 | 渲染流水线：CSS 如何影响首次加载时的白屏时间？》</a>这篇文章中分析过，渲染引擎会将所有的 CSS 内容解析为 CSSOM，在生成布局树的时候，会在 CSSOM 中为布局树中的元素查找样式，所以有两个相同标签最终所显示出来的效果是一样的，渲染引擎是不能为它们分别单独设置样式的。</p>
...
<h2 id="webcomponent-组件化开发">WebComponent 组件化开发</h2>
...
<p>前面我们说了，WebComponent 是一套技术的组合，具体涉及到了** Custom elements（自定义元素）、Shadow DOM（影子 DOM）和HTML templates（HTML 模板）**，详细内容你可以参考 MDN 上的<a href="https://developer.mozilla.org/zh-CN/docs/Web/Web_Components">相关链接</a>。</p>
...
<p><img src="/markdown-reader/images/28/579c65e2d2221f4e476c7846b842c27c.png" alt="使用影子 DOM 的输出效果"></p>
...
<h2 id="浏览器如何实现影子-dom">浏览器如何实现影子 DOM</h2>
<p>关于 WebComponent 的使用方式我们就介绍到这里。WebComponent 整体知识点不多，内容也不复杂，我认为核心就是影子 DOM。上面我们介绍影子 DOM 的作用主要有以下两点：</p>
<ol>
<li><p>影子 DOM 中的元素对于整个网页是不可见的；</p>
</li>
<li><p>影子 DOM 的 CSS 不会影响到整个网页的 CSSOM，影子 DOM 内部的 CSS 只对内部的元素起作用。</p>
</li>
</ol>
<p>那么浏览器是如何实现影子 DOM 的呢？下面我们就来分析下，如下图：</p>
<p><img src="/markdown-reader/images/28/5bce3d00c8139a7fde9cc90f9d803322.png" alt="影子 DOM 示意图"></p>
...
`,path:"/guide/28"};export{n as default};
