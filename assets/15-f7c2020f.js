const n={id:"15",title:"15 | 消息队列和事件循环：页面是怎么“活”起来的？",content:`<h1 id="15--消息队列和事件循环：页面是怎么活起来的？">15 | 消息队列和事件循环：页面是怎么“活”起来的？</h1>
...
<h2 id="使用单线程处理安排好的任务">使用单线程处理安排好的任务</h2>
...
<li><p>任务 4：打印出任务 1、任务 2、任务 3 的运算结果</p>
</li>
</ul>
<p>现在要在一个线程中去执行这些任务，通常我们会这样编写代码：</p>
<pre><code class="language-c++">void MainThread(){
  int num1 = 1+2; //任务1
  int num2 = 20/5; //任务2
  int num3 = 7*8; //任务3
...
<p><img src="/markdown-reader/images/15/72726678ac6604116c1d5dad160780bc.png" alt="第一版：线程的一次执行"></p>
<h2 id="在线程运行过程中处理新任务">在线程运行过程中处理新任务</h2>
...
<pre><code class="language-c++">//GetInput
...
<p>相较于第一版的线程，这一版的线程做了两点改进。<br>**</p>
<ul>
<li><p>第一点引入了循环机制**，具体实现方式是在线程语句最后添加了一个 <strong>for 循环语句</strong>，线程会一直循环执行。</p>
</li>
<li><p><strong>第二点是引入了事件</strong>，可以在线程运行过程中，等待用户输入的数字，等待过程中线程处于暂停状态，一旦接收到用户输入的信息，那么线程会被激活，然后执行相加运算，最后输出结果。</p>
</li>
</ul>
<p>通过引入事件循环机制，就可以让该线程“活”起来了，我们每次输入两个数字，都会打印出两数字相加的结果，你可以结合下图来参考下这个改进版的线程：</p>
<p><img src="/markdown-reader/images/15/9e0f595324fbd5b7cd1c1ae1140f7de3.png" alt="第二版：在线程中引入事件循环"></p>
<h2 id="处理其他线程发送过来的任务">处理其他线程发送过来的任务</h2>
...
<p><img src="/markdown-reader/images/15/2eb6a8ecb7cb528da4663573d74eb305.png" alt="渲染进程线程之间发送任务"></p>
...
<p><img src="/markdown-reader/images/15/6d141ec0925590d83d97a37cce8e6f5e.png" alt="消息队列"></p>
...
<p><img src="/markdown-reader/images/15/2ac6bc0361cb4690c5cc83d8abad22ab.png" alt="第三版线程模型：队列 + 循环"></p>
<p>从上图可以看出，我们的改造可以分为下面三个步骤：</p>
<ol>
<li><p>添加一个消息队列；</p>
</li>
<li><p>IO 线程中产生的新任务添加进消息队列尾部；</p>
</li>
<li><p>渲染主线程会循环地从消息队列头部中读取任务，执行任务。</p>
</li>
</ol>
<p>有了这些步骤之后，那么接下来我们就可以<strong>按步骤使用代码来实现第三版的线程模型。</strong></p>
<p>首先，构造一个队列。当然，在本篇文章中我们不需要考虑队列实现的细节，只是构造队列的接口：</p>
<pre><code class="language-c++">class TaskQueue{
  public:
...
<pre><code class="language-c++">TaskQueue task_queue；
void ProcessTask();
void MainThread(){
  for(;;){
...
<pre><code class="language-c++">Task clickTask;
task_queue.pushTask(clickTask)
</code></pre>
<p>由于是多个线程操作同一个消息队列，所以在添加任务和取出任务时还会加上一个<strong>同步锁</strong>，这块内容你也要注意下。</p>
<h2 id="处理其他进程发送过来的任务">处理其他进程发送过来的任务</h2>
<p>通过使用消息队列，我们实现了线程之间的消息通信。在 Chrome 中，跨进程之间的任务也是频繁发生的，那么如何处理其他进程发送过来的任务？你可以参考下图：</p>
<p><img src="/markdown-reader/images/15/e2582e980632fd2df5043f81a11461c6.png" alt="跨进程发送消息"></p>
...
<h2 id="消息队列中的任务类型">消息队列中的任务类型</h2>
<p>现在你知道页面主线程是如何接收外部任务的了，那接下来我们再来看看消息队列中的任务类型有哪些。你可以参考下<a href="https://source.chromium.org/chromium/chromium/src/+/main:third_party/blink/public/platform/task_type.h">Chromium 的官方源码</a>，这里面包含了很多内部消息类型，如输入事件（鼠标滚动、点击、移动）、微任务、文件读写、WebSocket、JavaScript 定时器等等。</p>
...
<h2 id="页面使用单线程的缺点">页面使用单线程的缺点</h2>
...
<p><img src="/markdown-reader/images/15/8de4b43fca99b180fdffe6a5af07b5cc.png" alt="单个任务执行时间过久"></p>
...
<h2 id="实践：浏览器页面是如何运行的">实践：浏览器页面是如何运行的</h2>
...
<ul>
<li><p>如果有一些确定好的任务，可以使用一个单线程来按照顺序处理这些任务，这是第一版线程模型。</p>
</li>
<li><p>要在线程执行过程中接收并处理新的任务，就需要引入循环语句和事件系统，这是第二版线程模型。</p>
</li>
<li><p>如果要接收其他线程发送过来的任务，就需要引入消息队列，这是第三版线程模型。</p>
</li>
<li><p>如果其他进程想要发送任务给页面主线程，那么先通过 IPC 把任务发送给渲染进程的 IO 线程，IO 线程再把任务发送给页面主线程。</p>
</li>
<li><p>消息队列机制并不是太灵活，为了适应效率和实时性，引入了微任务。</p>
</li>
</ul>
...
`,path:"/guide/15"};export{n as default};
