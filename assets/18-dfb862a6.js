const n={id:"18",title:"18 | 宏任务和微任务：不是所有任务都是一个待遇",content:`<h1 id="18--宏任务和微任务：不是所有任务都是一个待遇">18 | 宏任务和微任务：不是所有任务都是一个待遇</h1>
...
<ul>
<li><p>渲染事件（如解析 DOM、计算布局、绘制）；</p>
</li>
<li><p>用户交互事件（如鼠标点击、滚动页面、放大缩小等）；</p>
</li>
<li><p>JavaScript 脚本执行事件；</p>
</li>
<li><p>网络请求完成、文件读写完成事件。</p>
</li>
</ul>
...
<ul>
<li><p>先从多个消息队列中选出一个最老的任务，这个任务称为 oldestTask；</p>
</li>
<li><p>然后循环系统记录任务开始执行的时间，并把这个 oldestTask 设置为当前正在执行的任务；</p>
</li>
<li><p>当任务执行完成之后，删除当前正在执行的任务，并从对应的消息队列中删除掉这个 oldestTask；</p>
</li>
<li><p>最后统计执行完成的时长等信息。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/18/3c2b9b474c4df544df61ebd62a7b3715.png" alt="Performance 记录"></p>
...
<h2 id="微任务">微任务</h2>
<p>在理解了宏任务之后，下面我们就可以来看看什么是微任务了。在<a href="/markdown-reader/guide/17">上一篇文章</a>中，我们介绍过异步回调的概念，其主要有两种方式。</p>
...
<p><img src="/markdown-reader/images/18/839f468be3d683019c309e0acd8cd788.png" alt="微任务添加和执行流程示意图"></p>
...
<ul>
<li><p>微任务和宏任务是绑定的，每个宏任务在执行时，会创建自己的微任务队列。</p>
</li>
<li><p>微任务的执行时长会影响到当前宏任务的时长。比如一个宏任务在执行过程中，产生了 100 个微任务，执行每个微任务的时间是 10 毫秒，那么执行这 100 个微任务的时间就是 1000 毫秒，也可以说这 100 个微任务让宏任务的执行时间延长了 1000 毫秒。所以你在写代码的时候一定要注意控制微任务的执行时长。</p>
</li>
<li><p>在一个宏任务中，分别创建一个用于回调的宏任务和微任务，无论什么情况下，微任务都早于宏任务执行。</p>
</li>
</ul>
<h2 id="监听-dom-变化方法演变">监听 DOM 变化方法演变</h2>
...
`,path:"/guide/18"};export{n as default};
