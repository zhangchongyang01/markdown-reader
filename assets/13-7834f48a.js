const n={id:"13",title:"13 | 垃圾回收：垃圾数据是如何自动回收的？",content:`<h1 id="13--垃圾回收：垃圾数据是如何自动回收的？">13 | 垃圾回收：垃圾数据是如何自动回收的？</h1>
<p>在<a href="/markdown-reader/guide/12">上一篇文章</a>中，我们提到了 JavaScript 中的数据是如何存储的，并通过例子分析了<strong>原始数据类型是存储在栈空间中的，引用类型的数据是存储在堆空间中的</strong>。通过这种分配方式，我们解决了数据的内存分配的问题。</p>
...
<h2 id="不同语言的垃圾回收策略">不同语言的垃圾回收策略</h2>
...
<pre><code class="language-js">//在堆中分配内存
...
<p><img src="/markdown-reader/images/13/d807ca19c2c8853ef5a38dca0fb79ab0.jpg" alt="执行到 showName 函数时的内存模型"></p>
<p>从图中可以看出，原始类型的数据被分配到栈中，引用类型的数据会被分配到堆中。当 foo 函数执行结束之后，foo 函数的执行上下文会从堆中被销毁掉，那么它是怎么被销毁的呢？下面我们就来分析一下。</p>
<p>在<a href="/markdown-reader/guide/12.md">上篇文章</a>中，我们简单介绍过了，如果执行到 showName 函数时，那么 JavaScript 引擎会创建 showName 函数的执行上下文，并将 showName 函数的执行上下文压入到调用栈中，最终执行到 showName 函数时，其调用栈就如上图所示。与此同时，还有一个<strong>记录当前执行状态的指针（称为 ESP）</strong>，指向调用栈中 showName 函数的执行上下文，表示当前正在执行 showName 函数。</p>
...
<p><img src="/markdown-reader/images/13/b899cb27c0d92c31f9377db59939aaf3.jpg" alt="从栈中回收 showName 执行上下文"></p>
...
<p><img src="/markdown-reader/images/13/e80ff553417572f77973b08256b6928c.png" alt="foo 函数执行结束后的内存状态"></p>
...
<h2 id="代际假说和分代收集">代际假说和分代收集</h2>
<p>不过在正式介绍 V8 是如何实现回收之前，你需要先学习下<strong>代际假说（The Generational Hypothesis）</strong>的内容，这是垃圾回收领域中一个重要的术语，后续垃圾回收的策略都是建立在该假说的基础之上的，所以很是重要。</p>
...
<h2 id="垃圾回收器的工作流程">垃圾回收器的工作流程</h2>
...
<p><img src="/markdown-reader/images/13/4f9310c7da631fa5a57f871099bfbeaf.png" alt="新生区要划分为对象区域和空闲区域"></p>
...
<p>由于老生区的对象比较大，若要在老生区中使用 Scavenge 算法进行垃圾回收，复制这些大的对象将会花费比较多的时间，从而导致回收执行效率不高，同时还会浪费一半的空间。因而，主垃圾回收器是采用<strong>标记 - 清除（Mark-Sweep）</strong>的算法进行垃圾回收的。下面我们来看看该算法是如何工作的。</p>
...
<p><img src="/markdown-reader/images/13/6c8361d3e52c1c37a06699ed94652e69.png" alt="标记过程"></p>
...
<p><img src="/markdown-reader/images/13/d015db8ad0df7f0ccb1bdb8e31f96e85.png" alt="标记清除过程"></p>
...
<p><img src="/markdown-reader/images/13/652bd2df726d0aa5e67fe8489f39a18c.png" alt="标记整理过程"></p>
<h2 id="全停顿">全停顿</h2>
...
<p><img src="/markdown-reader/images/13/9898646a08b46bce4f12f918f3c1e60c.png" alt="全停顿"></p>
...
<p>为了降低老生代的垃圾回收而造成的卡顿，V8 将标记过程分为一个个的子标记过程，同时让垃圾回收标记和 JavaScript 应用逻辑交替进行，直到标记阶段完成，我们把这个算法称为<strong>增量标记（Incremental Marking）</strong>算法。如下图所示：</p>
<p><img src="/markdown-reader/images/13/de117fc96ae425ed90366e9060aa14e7.png" alt="增量标记"></p>
...
`,path:"/guide/13"};export{n as default};
