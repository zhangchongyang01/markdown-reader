const e={id:"34",title:"34 | CSRF攻击：陌生链接不要随便点",content:`<h1 id="34--csrf攻击：陌生链接不要随便点">34 | CSRF攻击：陌生链接不要随便点</h1>
<p>在<a href="/markdown-reader/guide/33">上一篇文章</a>中我们讲到了 XSS 攻击，XSS 的攻击方式是黑客往用户的页面中注入恶意脚本，然后再通过恶意脚本将用户页面的数据上传到黑客的服务器上，最后黑客再利用这些数据进行一些恶意操作。XSS 攻击能够带来很大的破坏性，不过另外一种类型的攻击也不容忽视，它就是我们今天要聊的 CSRF 攻击。</p>
...
<p><img src="/markdown-reader/images/34/3d7f097b1d6a8f93a960a12892f1556b.png" alt="David 域名被盗流程"></p>
<ul>
<li><p>首先 David 发起登录 Gmail 邮箱请求，然后 Gmail 服务器返回一些登录状态给 David 的浏览器，这些信息包括了 Cookie、Session 等，这样在 David 的浏览器中，Gmail 邮箱就处于登录状态了。</p>
</li>
<li><p>接着黑客通过各种手段引诱 David 去打开他的链接，比如 hacker.com，然后在 hacker.com 页面中，黑客编写好了一个邮件过滤器，并通过 Gmail 提供的 HTTP 设置接口设置好了新的邮件过滤功能，该过滤器会将 David 所有的邮件都转发到黑客的邮箱中。</p>
</li>
<li><p>最后的事情就很简单了，因为有了 David 的邮件内容，所以黑客就可以去域名服务商那边重置 David 域名账户的密码，重置好密码之后，就可以将其转出到黑客的账户了。</p>
</li>
<li><p>以上就是 David 的域名被盗的完整过程，其中前两步就是我们今天要聊的 CSRF 攻击。David 在要回了他的域名之后，也将整个攻击过程分享到他的站点上了，如果你感兴趣的话，可以参考<a href="https://www.davidairey.com/google-gmail-security-hijack">该链接</a>（放心这个链接是安全的）。</p>
</li>
</ul>
...
<h3 id="1-自动发起-get-请求">1. 自动发起 Get 请求</h3>
...
<h3 id="2-自动发起-post-请求">2. 自动发起 POST 请求</h3>
...
<h3 id="3-引诱用户点击链接">3. 引诱用户点击链接</h3>
...
<ul>
<li><p>第一个，目标站点一定要有 CSRF 漏洞；</p>
</li>
<li><p>第二个，用户要登录过目标站点，并且在浏览器上保持有该站点的登录状态；</p>
</li>
<li><p>第三个，需要用户打开一个第三方站点，可以是黑客的站点，也可以是一些论坛。</p>
</li>
</ul>
...
<h3 id="1-充分利用好-cookie-的-samesite-属性">1. 充分利用好 Cookie 的 SameSite 属性</h3>
...
<ul>
<li><p>如果是从第三方站点发起的请求，那么需要浏览器禁止发送某些关键 Cookie 数据到服务器；</p>
</li>
<li><p>如果是同一个站点发起的请求，那么就需要保证 Cookie 数据正常发送。</p>
</li>
</ul>
...
<pre><code>set-cookie: 1P_JAR=2019-10-20-06; expires=Tue, 19-Nov-2019 06:36:21 GMT; path=/; domain=.google.com; SameSite=none
</code></pre>
<p>SameSite 选项通常有 Strict、Lax 和 None 三个值。</p>
<ul>
<li><p>Strict 最为严格。如果 SameSite 的值是 Strict，那么浏览器会完全禁止第三方 Cookie。简言之，如果你从极客时间的页面中访问 InfoQ 的资源，而 InfoQ 的某些 Cookie 设置了 SameSite = Strict 的话，那么这些 Cookie 是不会被发送到 InfoQ 的服务器上的。只有你从 InfoQ 的站点去请求 InfoQ 的资源时，才会带上这些 Cookie。</p>
</li>
<li><p>Lax 相对宽松一点。在跨站点的情况下，从第三方站点的链接打开和从第三方站点提交 Get 方式的表单这两种方式都会携带 Cookie。但如果在第三方站点中使用 Post 方法，或者通过 img、iframe 等标签加载的 URL，这些场景都不会携带 Cookie。</p>
</li>
<li><p>而如果使用 None 的话，在任何情况下都会发送 Cookie 数据。</p>
</li>
</ul>
<p>关于 SameSite 的具体使用方式，你可以参考这个链接：<a href="https://web.dev/samesite-cookies-explained">https://web.dev/samesite-cookies-explained</a> 。</p>
<p>对于防范 CSRF 攻击，我们可以针对实际情况将一些关键的 Cookie 设置为 Strict 或者 Lax 模式，这样在跨站点请求时，这些关键的 Cookie 就不会被发送到服务器，从而使得黑客的 CSRF 攻击失效。</p>
<h3 id="2-验证请求的来源站点">2. 验证请求的来源站点</h3>
...
<p><img src="/markdown-reader/images/34/159430e9d15cb7bcfa4fd014da31a2c9.png" alt="HTTP 请求头中的 Referer 引用"></p>
...
<p><img src="/markdown-reader/images/34/258dc5542db8961aaa23ec0c02030003.png" alt="Post 请求时的 Origin 信息"></p>
...
<p>作者: </p>
<p>我把整个流程写一遍：<br>首先假设你发出登录InfoQ的站点请求，然后在InfoQ返回HTTP响应头给浏览器，InfoQ响应头中的某些set-cookie字段如下所示：<br>set-cookie: a_value=avalue_xxx; expires=Thu, 21-Nov-2019 03:53:16 GMT; path=/; domain=.infoq.com; SameSite=strict<br>set-cookie: b_value=bvalue_xxx; expires=Thu, 21-Nov-2019 03:53:16 GMT; path=/; domain=.infoq.com; SameSite=lax<br>set-cookie: c_value=cvaule_xxx; expires=Thu, 21-Nov-2019 03:53:16 GMT; path=/; domain=.infoq.com; SameSite=none<br>set-cookie: d_value=dvaule_xxxx; expires=Thu, 21-Nov-2019 03:53:16 GMT; path=/; domain=.infoq.com; </p>
<p>我们可以看出，<br>a_value的SameSite属性设置成了strict，<br>b_value的SameSite属性设置成了lax<br>c_value的SameSite属性值设置成了none<br>d_value没有设置SameSite属性值</p>
<p>好，这些Cookie设置好之后，当你再次在InfoQ的页面内部请求InfoQ的资源时，这些Cookie信息都会被附加到HTTP的请求头中，如下所示：<br>cookie: a_value=avalue_xxx;b_value=bvalue_xxx;c_value=cvaule_xxx;d_value=dvaule_xxxx;</p>
<p>但是，假如你从time.geekbang.org的页面中，通过a标签打开页面，如下所示：<br><code>&lt;a href=&quot;https://www.infoq.cn/sendcoin?user=hacker&amp;number=100&quot;&gt;点我下载&lt;/a&gt;</code><br>当用户点击整个链接的时候，因为InfoQ中a_vaule的SameSite的值设置成了strict，那么a_vaule的值将不会被携带到这个请求的HTTP头中。</p>
<p>如果time.geekbang.org的页面中，有通过img来加载的infoq的资源代码，如下所示：<br><code>&lt;img src=&quot;https://www.infoq.cn/sendcoin?user=hacker&amp;number=100&quot;&gt;</code><br>那么在加载infoQ资源的时候，只会携带c_value,和d_value的值。</p>
<p>这样写不知道你明白没有，如果还有疑惑欢迎继续提问。</p>
`,path:"/guide/34"};export{e as default};
