const n={id:"05",title:"05 | 渲染流程（上）：HTML、CSS和JavaScript，是如何变成页面的？",content:`<h1 id="05--渲染流程（上）：html、css和javascript，是如何变成页面的？">05 | 渲染流程（上）：HTML、CSS和JavaScript，是如何变成页面的？</h1>
<p>在<a href="/markdown-reader/guide/04">上一篇文章</a>中我们介绍了导航相关的流程，那导航被提交后又会怎么样呢？就进入了渲染阶段。这个阶段很重要，了解其相关流程能让你“看透”页面是如何工作的，有了这些知识，你可以解决一系列相关的问题，比如能熟练使用开发者工具，因为能够理解开发者工具里面大部分项目的含义，能优化页面卡顿问题，使用 JavaScript 优化动画流程，通过优化样式表来防止强制同步布局，等等。</p>
...
<p><img src="/markdown-reader/images/05/2b08a85c63bee68c6fd95dabb648fd79.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/31cd7172f743193d682d088a60cb44e6.png" alt="img"></p>
<p>从上图可以看出，<strong>HTML 的内容是由标记和文本组成</strong>。标记也称为<strong>标签</strong>，每个标签都有它自己的语义，浏览器会根据标签的语义来正确展示 HTML 内容。比如上面的<code>&lt;p&gt;</code>标签是告诉浏览器在这里的内容需要创建一个新段落，中间的文本就是段落中需要显示的内容。</p>
<p>如果需要改变 HTML 的字体颜色、大小等信息，就需要用到 CSS。CSS 又称为<strong>层叠样式表</strong>，<strong>是由选择器和属性组成</strong>，比如图中的 p 选择器，它会把 HTML 里面<code>&lt;p&gt;</code>标签的内容选择出来，然后再把选择器的属性值应用到<code>&lt;p&gt;</code>标签内容上。选择器里面有个 color 属性，它的值是 red，这是告诉渲染引擎把<code>&lt;p&gt;</code>标签的内容显示为红色。</p>
...
<p><img src="/markdown-reader/images/05/9259f8732ddad472e5e08a633ad46de8.png" alt="img"></p>
<p>按照渲染的时间顺序，流水线可分为如下几个子阶段：构建 DOM 树、样式计算、布局阶段、分层、绘制、分块、光栅化和合成。内容比较多，我会用两篇文章来为你详细讲解这各个子阶段。接下来，在介绍每个阶段的过程中，你应该重点关注以下三点内容：</p>
<ul>
<li><p>开始每个子阶段都有其<strong>输入的内容</strong>；</p>
</li>
<li><p>然后每个子阶段有其<strong>处理过程</strong>；</p>
</li>
<li><p>最终每个子阶段会生成<strong>输出内容</strong>。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/05/fcad0a4e3e73c796f00d6120284a3638.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/125849ec56a3ea98d4b476c66c754f79.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/47f57c3eee749dd838939bfe5dd64573.png" alt="img"></p>
...
<pre><code>document.getElementsByTagName(&quot;p&quot;)[0].innerText = &quot;black&quot;
</code></pre>
<p>这行代码的作用是把第一个<code>&lt;p&gt;</code>标签的内容修改为 black，具体执行结果你可以参考下图：</p>
<p><img src="/markdown-reader/images/05/e730aa1d73c1151c588e2f8c7e22c274.png" alt="img"></p>
...
<h2 id="样式计算（recalculate-style）">样式计算（Recalculate Style）</h2>
<p>样式计算的目的是为了计算出 DOM 节点中每个元素的具体样式，这个阶段大体可分为三步来完成。</p>
<h3 id="1-把-css-转换为浏览器能够理解的结构">1. 把 CSS 转换为浏览器能够理解的结构</h3>
<p>那 CSS 样式的来源主要有哪些呢？你可以先参考下图：</p>
<p><img src="/markdown-reader/images/05/bc93df7b8d03b2675f21e1d9e4e1407c.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/8ec7d5ecfadcd05b3f1ec762223a9aab.png" alt="img"></p>
...
<p>可以看到上面的 CSS 文本中有很多属性值，如 2em、blue、bold，这些类型数值不容易被渲染引擎理解，所以<strong>需要将所有值转换为渲染引擎容易理解的、标准化的计算值</strong>，这个过程就是属性值标准化。</p>
<p>那标准化后的属性值是什么样子的？</p>
<p><img src="/markdown-reader/images/05/1252c6d3c1a51714606daa6bdad3a560.png" alt="img"></p>
<p>从图中可以看到，2em 被解析成了 32px，red 被解析成了 rgb(255,0,0)，bold 被解析成了 700……</p>
<h3 id="3-计算出-dom-树中每个节点的具体样式">3. 计算出 DOM 树中每个节点的具体样式</h3>
...
<p><img src="/markdown-reader/images/05/fe9a0ea868dc02a3c4a59f6080aa80b4.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/88a3aac427cc7c09361eac01a85fc7b2.png" alt="img"></p>
<p>这个界面展示的信息很丰富，大致可描述为如下。</p>
<ul>
<li><p>首先，可以选择要<strong>查看的元素的样式（位于图中的区域 2 中）</strong>，在图中的第 1 个区域中点击对应的元素，就可以在下面的区域查看该元素的样式了。比如这里我们选择的元素是<code>&lt;p&gt;</code>标签，位于 html.body.div. 这个路径下面。</p>
</li>
<li><p>其次，可以从<strong>样式来源（位于图中的区域 3 中）</strong>中查看样式的具体来源信息，看看是来源于样式文件，还是来源于 UserAgent 样式表。<strong>这里需要特别提下 UserAgent 样式，它是浏览器提供的一组默认样式，如果你不提供任何样式，默认使用的就是 UserAgent 样式。</strong></p>
</li>
<li><p>最后，可以通过区域 2 和区域 3 来查看样式继承的具体过程。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/05/d87415b0187e3860404bf963f1c3d646.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/8e48b77dd48bdc509958e73b9935710e.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/05/a4a0ea4da58260aafc9aabdd37613f9a.png" alt="img"></p>
<p>从图中可以看出，本节内容我们介绍了渲染流程的前三个阶段：DOM 生成、样式计算和布局。要点可大致总结为如下：</p>
<ul>
<li><p>浏览器不能直接理解 HTML 数据，所以第一步需要将其转换为浏览器能够理解的 DOM 树结构；</p>
</li>
<li><p>生成 DOM 树后，还需要根据 CSS 样式表，来计算出 DOM 树所有节点的样式；</p>
</li>
</ul>
...
`,path:"/guide/05"};export{n as default};
