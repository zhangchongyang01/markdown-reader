const n={id:"06",title:"06 | 渲染流程（下）：HTML、CSS和JavaScript，是如何变成页面的？",content:`<h1 id="06--渲染流程（下）：html、css和javascript，是如何变成页面的？">06 | 渲染流程（下）：HTML、CSS和JavaScript，是如何变成页面的？</h1>
<p>在<a href="/markdown-reader/guide/05">上篇文章</a>中，我们介绍了渲染流水线中的 <strong>DOM 生成、样式计算和布局</strong>三个阶段，那今天我们接着讲解渲染流水线后面的阶段。</p>
...
<p><img src="/markdown-reader/images/06/e2c917edf5119cddfbec9481372f8fc0.png" alt="img"></p>
<p>从上图可以看出，渲染引擎给页面分了很多图层，这些图层按照一定顺序叠加在一起，就形成了最终的页面，你可以参考下图：</p>
<p><img src="/markdown-reader/images/06/cd6aac705501d48bda6e8eebca058b78.png" alt="img"></p>
<p>现在你知道了<strong>浏览器的页面实际上被分成了很多图层，这些图层叠加后合成了最终的页面</strong>。下面我们再来看看这些图层和布局树节点之间的关系，如文中图所示：</p>
<p><img src="/markdown-reader/images/06/e8a7e60a2a08e05239456284d2aa4061.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/a03eb12053aac1ac496b61a424f20119.png" alt="img"></p>
<p>从图中可以看出，明确定位属性的元素、定义透明属性的元素、使用 CSS 滤镜的元素等，都拥有层叠上下文属性。</p>
<p>若你想要了解更多层叠上下文的知识，你可以参考<a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_Positioning/Understanding_z_index/The_stacking_context">这篇文章</a>。</p>
...
<p><img src="/markdown-reader/images/06/6a583733735edc1e4d7946740eb6fc0c.png" alt="img"></p>
<p>出现这种裁剪情况的时候，渲染引擎会为文字部分单独创建一个层，如果出现滚动条，滚动条也会被提升为单独的层。你可以参考下图：</p>
<p><img src="/markdown-reader/images/06/7b6ceaab23c6c6d8e5930864ff9d7097.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/40825a55214a7990bba6b9bec6e54108.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/303515c26fcd4eaa9b9966ad7f190370.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/46d33b6e5fca889ecbfab4516c80a441.png" alt="img"></p>
<p>如上图所示，当图层的绘制列表准备好之后，主线程会把该绘制列表<strong>提交（commit）</strong>给合成线程，那么接下来合成线程是怎么工作的呢？</p>
<p>那我们得先来看看什么是视口，你可以参看下图：</p>
<p><img src="/markdown-reader/images/06/242225112f2a3ec97e736c960b88d972.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/bcc7f6983d5ece8e2dd716f431d0e052.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/d8d77356211e12b47bb9f508e2db8520.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/a8d954cd8e4722ee03d14afaa14c3987.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/975fcbf7f83cc20d216f3d68a85d0f37.png" alt="img"></p>
<p>结合上图，一个完整的渲染流程大致可总结为如下：</p>
<ol>
<li><p>渲染进程将 HTML 内容转换为能够读懂的 DOM 树结构。</p>
</li>
<li><p>渲染引擎将 CSS 样式表转化为浏览器可以理解的 styleSheets，计算出 DOM 节点的样式。</p>
</li>
<li><p>创建布局树，并计算元素的布局信息。</p>
</li>
<li><p>对布局树进行分层，并生成分层树。</p>
</li>
<li><p>为每个图层生成绘制列表，并将其提交到合成线程。</p>
</li>
<li><p>合成线程将图层分成图块，并在光栅化线程池中将图块转换成位图。</p>
</li>
<li><p>合成线程发送绘制图块命令 DrawQuad 给浏览器进程。</p>
</li>
<li><p>浏览器进程根据 DrawQuad 消息生成页面，并显示到显示器上。</p>
</li>
</ol>
<h2 id="相关概念">相关概念</h2>
...
<p>你可先参考下图：</p>
<p><img src="/markdown-reader/images/06/b3ed565230fe4f5c1886304a8ff754e5.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/3c1b7310648cccbf6aa4a42ad0202b03.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/06/024bf6c83b8146d267f476555d953a2c.png" alt="img"></p>
...
`,path:"/guide/06"};export{n as default};
