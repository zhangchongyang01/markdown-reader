const p={id:"33",title:"33 | 跨站脚本攻击（XSS）：为什么Cookie中有HttpOnly属性？",content:`<h1 id="33--跨站脚本攻击（xss）：为什么cookie中有httponly属性？">33 | 跨站脚本攻击（XSS）：为什么Cookie中有HttpOnly属性？</h1>
<p>通过<a href="/markdown-reader/guide/32">上篇文章</a>的介绍，我们知道了同源策略可以隔离各个站点之间的 DOM 交互、页面数据和网络通信，虽然严格的同源策略会带来更多的安全，但是也束缚了 Web。这就需要在安全和自由之间找到一个平衡点，所以我们默认页面中可以引用任意第三方资源，然后又引入 CSP 策略来加以限制；默认 XMLHttpRequest 和 Fetch 不能跨站请求资源，然后又通过 CORS 策略来支持其跨域。</p>
...
<ul>
<li><p>可以<strong>窃取 Cookie 信息</strong>。恶意 JavaScript 可以通过“document.cookie”获取 Cookie 信息，然后通过 XMLHttpRequest 或者 Fetch 加上 CORS 功能将数据发送给恶意服务器；恶意服务器拿到用户的 Cookie 信息之后，就可以在其他电脑上模拟用户的登录，然后进行转账等操作。</p>
</li>
<li><p>可以<strong>监听用户行为</strong>。恶意 JavaScript 可以使用“addEventListener”接口来监听键盘事件，比如可以获取用户输入的信用卡等信息，将其发送到恶意服务器。黑客掌握了这些信息之后，又可以做很多违法的事情。</p>
</li>
<li><p>可以通过<strong>修改 DOM</strong> 伪造假的登录窗口，用来欺骗用户输入用户名和密码等信息。</p>
</li>
<li><p>还可以<strong>在页面内生成浮窗广告</strong>，这些广告会严重地影响用户体验。</p>
</li>
</ul>
...
<h3 id="1-存储型-xss-攻击">1. 存储型 XSS 攻击</h3>
<p>我们先来看看存储型 XSS 攻击是怎么向 HTML 文件中注入恶意脚本的，你可以参考下图：</p>
<p><img src="/markdown-reader/images/33/2ed3d8b93035df3c2bcfcc223dc47914.png" alt="存储型 XSS 攻击"></p>
<p>通过上图，我们可以看出存储型 XSS 攻击大致需要经过如下步骤：</p>
<ul>
<li><p>首先黑客利用站点漏洞将一段恶意 JavaScript 代码提交到网站的数据库中；</p>
</li>
<li><p>然后用户向网站请求包含了恶意 JavaScript 脚本的页面；</p>
</li>
<li><p>当用户浏览该页面的时候，恶意脚本就会将用户的 Cookie 信息等数据上传到服务器。</p>
</li>
</ul>
<p>下面我们来看个例子，2015 年喜马拉雅就被曝出了存储型 XSS 漏洞。起因是在用户设置专辑名称时，服务器对关键字过滤不严格，比如可以将专辑名称设置为一段 JavaScript，如下图所示：</p>
<p><img src="/markdown-reader/images/33/5479e94a06d9a7cdf3920c60bf834249.png" alt="黑客将恶意代码存储到漏洞服务器上"></p>
...
<p><img src="/markdown-reader/images/33/efaf4123438f37da4c7366b87ed1403a.png" alt="用户打开了含有恶意脚本的页面"></p>
...
<p><img src="/markdown-reader/images/33/b19300e39a753774f4a94635b46af7cb.png" alt="将 Cookie 等数据上传到黑客服务器"></p>
<p>黑客拿到了用户 Cookie 信息之后，就可以利用 Cookie 信息在其他机器上登录该用户的账号（如下图），并利用用户账号进行一些恶意操作。</p>
<p><img src="/markdown-reader/images/33/720c6daf21a8adf55329af9eaa5ab052.png" alt="黑客利用 Cookie 信息登录用户账户"></p>
<p>以上就是存储型 XSS 攻击的一个典型案例，这是乌云网在 2015 年曝出来的，虽然乌云网由于某些原因被关停了，但是你依然可以通过这个站点来查看乌云网的一些备份信息。</p>
<h3 id="2-反射型-xss-攻击">2. 反射型 XSS 攻击</h3>
...
<p>上面这两段代码，第一段是路由，第二段是视图，作用是将 URL 中 xss 参数的内容显示在页面。我们可以在本地演示下，比如打开<a href="http://localhost:3000/?xss=123%E8%BF%99%E4%B8%AA%E9%93%BE%E6%8E%A5%EF%BC%8C%E8%BF%99%E6%A0%B7%E5%9C%A8%E9%A1%B5%E9%9D%A2%E4%B8%AD%E5%B1%95%E7%A4%BA%E5%B0%B1%E6%98%AF%E2%80%9C123%E2%80%9D%E4%BA%86%EF%BC%88%E5%A6%82%E4%B8%8B%E5%9B%BE%EF%BC%89%EF%BC%8C%E6%98%AF%E6%AD%A3%E5%B8%B8%E7%9A%84%EF%BC%8C%E6%B2%A1%E6%9C%89%E9%97%AE%E9%A2%98%E7%9A%84%E3%80%82">http://localhost:3000/?xss=123这个链接，这样在页面中展示就是“123”了（如下图），是正常的，没有问题的。</a></p>
<p><img src="/markdown-reader/images/33/7ecb717abadfff2637a168d39f0c3cdb.png" alt="正常打开页面"></p>
<p>但当打开<code>http://localhost:3000/?xss=&lt;script&gt;alert(&#39;你被xss攻击了&#39;)&lt;/script&gt;</code>这段 URL 时，其结果如下图所示：</p>
<p><img src="/markdown-reader/images/33/4dff7d83fe2eecc6cb52c126b4f650fa.png" alt="反射型 XSS 攻击"></p>
...
<h3 id="3-基于-dom-的-xss-攻击">3. 基于 DOM 的 XSS 攻击</h3>
...
<h2 id="如何阻止-xss-攻击">如何阻止 XSS 攻击</h2>
...
<h3 id="1-服务器对输入脚本进行过滤或转码">1. 服务器对输入脚本进行过滤或转码</h3>
<p>不管是反射型还是存储型 XSS 攻击，我们都可以在服务器端将一些关键的字符进行转码，比如最典型的：</p>
<pre><code class="language-html">code:&lt;script&gt;alert(&#39;你被xss攻击了&#39;)&lt;/script&gt;
</code></pre>
<p>这段代码过滤后，只留下了：</p>
<pre><code>code:
</code></pre>
...
<pre><code>code:&amp;lt;script&amp;gt;alert(&amp;#39;你被xss攻击了&amp;#39;)&amp;lt;/script&amp;gt;
</code></pre>
<p>经过转码之后的内容，如<code>&lt;script&gt;</code>标签被转换为&lt;script&gt;，因此即使这段脚本返回给页面，页面也不会执行这段脚本。</p>
<h3 id="2-充分利用-csp">2. 充分利用 CSP</h3>
...
<ul>
<li><p>限制加载其他域下的资源文件，这样即使黑客插入了一个 JavaScript 文件，这个 JavaScript 文件也是无法被加载的；</p>
</li>
<li><p>禁止向第三方域提交数据，这样用户数据也不会外泄；</p>
</li>
<li><p>禁止执行内联脚本和未授权的脚本；</p>
</li>
<li><p>还提供了上报机制，这样可以帮助我们尽快发现有哪些 XSS 攻击，以便尽快修复问题。</p>
</li>
</ul>
<p>因此，利用好 CSP 能够有效降低 XSS 攻击的概率。<a href="https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CSP">内容安全策略 - MDN文档</a></p>
<h3 id="3-使用-httponly-属性">3. 使用 HttpOnly 属性</h3>
...
<pre><code>set-cookie: NID=189=M8q2FtWbsR8RlcldPVt7qkrqR38LmFY9jUxkKo3-4Bi6Qu_ocNOat7nkYZUTzolHjFnwBw0izgsATSI7TZyiiiaV94qGh-BzEYsNVa7TZmjAYTxYTOM9L_-0CN9ipL6cXi8l6-z41asXtm2uEwcOC5oh9djkffOMhWqQrlnCtOI; expires=Sat, 18-Apr-2020 06:52:22 GMT; path=/; domain=.google.com; HttpOnly
</code></pre>
...
<p><img src="/markdown-reader/images/33/defa78c90a4e8f0debb09564561ab9bb.png" alt="HttpOnly 演示"></p>
...
`,path:"/guide/33"};export{p as default};
