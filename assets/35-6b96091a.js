const p={id:"35",title:"35 | 安全沙箱：页面和系统之间的隔离墙",content:`<h1 id="35--安全沙箱：页面和系统之间的隔离墙">35 | 安全沙箱：页面和系统之间的隔离墙</h1>
<p>前面三篇文章我们主要围绕同源策略介绍了 Web 页面安全的相关内容，那今天我们把视野向外延伸，来聊聊页面安全和操作系统安全之间的关系。</p>
<p>在<a href="/markdown-reader/guide/01">《01 | Chrome 架构：仅仅打开了 1 个页面，为什么有 4 个进程？》</a>那篇文章中，我们分析了浏览器架构的发展史，在最开始的阶段，浏览器是单进程的，这意味着渲染过程、JavaScript 执行过程、网络加载过程、UI 绘制过程和页面显示过程等都是在同一个进程中执行的，这种结构虽然简单，但是也带来了很多问题。</p>
...
<ul>
<li><p>XSS 攻击只是将恶意的 JavaScript 脚本注入到页面中，虽然能窃取一些 Cookie 相关的数据，但是 XSS 无法对操作系统进行攻击。</p>
</li>
<li><p>而通过浏览器漏洞进行的攻击是可以入侵到浏览器进程内部的，可以读取和修改浏览器进程内部的任意内容，还可以穿透浏览器，在用户的操作系统上悄悄地安装恶意软件、监听用户键盘输入信息以及读取用户硬盘上的文件内容。</p>
</li>
</ul>
...
<p>我们知道现代浏览器采用了多进程架构，将渲染进程和浏览器主进程做了分离，完整的进程架构我们已经在<a href="/markdown-reader/guide/01">《01 | Chrome 架构：仅仅打开了 1 个页面，为什么有 4 个进程？》</a>那篇文章中介绍过了，这里我就不重复介绍了。下面我们重点从操作系统安全的视角来看看浏览器的多进程架构，如下图：</p>
<p><img src="/markdown-reader/images/35/b83693a1ace43f43f9cab242982de6b1.png" alt="浏览器内核和渲染进程"></p>
...
<p>在<a href="/markdown-reader/guide/01">《01 | Chrome 架构：仅仅打开了 1 个页面，为什么有 4 个进程？》</a>中我们分析过，设计现代浏览器体系架构时，将浏览器划分为不同的进程是为了增加其稳定性。虽然设计成了多进程架构，不过这些模块之间的沟通方式却有些复杂，也许你还有以下问题：</p>
<ul>
<li><p>为什么一定要通过浏览器内核去请求资源，再将数据转发给渲染进程，而不直接从进程内部去请求网络资源？</p>
</li>
<li><p>为什么渲染进程只负责生成页面图片，生成图片还要经过 IPC 通知浏览器内核模块，然后让浏览器内核去负责展示图片？</p>
</li>
</ul>
...
<h2 id="安全沙箱如何影响各个模块功能">安全沙箱如何影响各个模块功能</h2>
...
<p><img src="/markdown-reader/images/35/f8cc6394832ed238f18a01eff5726f1b.png" alt="浏览器内核和渲染进程各自职责"></p>
<p>通过该图，我们可以看到由于渲染进程需要安全沙箱的保护，因此需要把在渲染进程内部涉及到和系统交互的功能都转移到浏览器内核中去实现。</p>
<p>那安全沙箱是如何影响到各个模块功能的呢？</p>
<h3 id="1-持久存储">1. 持久存储</h3>
...
<ul>
<li><p>存储 Cookie 数据的读写。通常浏览器内核会维护一个存放所有 Cookie 的 Cookie 数据库，然后当渲染进程通过 JavaScript 来读取 Cookie 时，渲染进程会通过 IPC 将读取 Cookie 的信息发送给浏览器内核，浏览器内核读取 Cookie 之后再将内容返回给渲染进程。</p>
</li>
<li><p>一些缓存文件的读写也是由浏览器内核实现的，比如网络文件缓存的读取。</p>
</li>
</ul>
<h3 id="2-网络访问">2. 网络访问</h3>
...
<h2 id="站点隔离（site-isolation）">站点隔离（Site Isolation）</h2>
...
<p>值得注意是，2019 年 10 月 20 日 Chrome 团队宣布安卓版的 Chrome 已经全面支持站点隔离，你可以参考<a href="https://www.digitalinformationworld.com/2019/10/google-improves-site-isolation-for-stronger-chrome-browser-security.html">文中链接</a>。</p>
<h2 id="总结">总结</h2>
...
<h2 id="参考资料">参考资料</h2>
<ol>
<li><p>安全沙箱的设计参考了<a href="https://zh.wikipedia.org/wiki/%E6%9C%80%E5%B0%8F%E6%9D%83%E9%99%90%E5%8E%9F%E5%88%99">最小权限原则</a></p>
</li>
<li><p><a href="http://seclab.stanford.edu/websec/chromium/chromium-security-architecture.pdf">The Security Architecture of the Chromium Browser</a></p>
</li>
<li><p><a href="https://prezi.com/l3zlqveefln7/the-security-architecture-of-the-chromium-browser/">The Security Architecture of the Chromium Browser-ppt</a></p>
</li>
<li><p><a href="https://www.chromium.org/developers/design-documents/site-isolation/">chromium site-isolation</a></p>
</li>
<li><p><a href="https://www.bisend.cn/blog/google-chrome-site-isolation">Site Isolation</a></p>
</li>
<li><p><a href="https://www.usenix.org/system/files/sec19-reis.pdf">Site Isolation: Process Separation for Web Sites within the Browser</a></p>
</li>
</ol>
`,path:"/guide/35"};export{p as default};
