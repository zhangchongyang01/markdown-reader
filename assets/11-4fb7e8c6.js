const n={id:"11",title:"11 | this：从JavaScript执行上下文的视角讲清楚this",content:`<h1 id="11--this：从javascript执行上下文的视角讲清楚this">11 | this：从JavaScript执行上下文的视角讲清楚this</h1>
<p>在<a href="/markdown-reader/guide/10">上篇文章</a>中，我们讲了词法作用域、作用域链以及闭包，并在最后思考题中留了下面这样一段代码：</p>
...
<p>相信你已经知道了，在 printName 函数里面使用的变量 myName 是属于全局作用域下面的，所以最终打印出来的值都是“极客邦”。这是因为 JavaScript 语言的作用域链是由词法作用域决定的，而词法作用域是由代码结构来确定的。</p>
...
<p>在这段 C++ 代码中，我同样调用了 bar 对象中的 printName 方法，最后打印出来的值就是 bar 对象的内部变量 myName 值——“time.geekbang.com”，而并不是最外面定义变量 myName 的值——“极客邦”，所以<strong>在对象内部的方法中使用对象内部的属性是一个非常普遍的需求</strong>。但是 JavaScript 的作用域机制并不支持这一点，基于这个需求，JavaScript 又搞出来另外一套 <strong>this 机制</strong>。</p>
<p>所以，在 JavaScript 中可以使用 this 实现在 printName 函数中访问到 bar 对象的 myName 属性了。具体该怎么操作呢？你可以调整 printName 的代码，如下所示：</p>
<pre><code class="language-js">printName: function () {
  console.log(this.myName)
}
</code></pre>
<p>接下来咱们就展开来介绍 this，不过在讲解之前，希望你能区分清楚<strong>作用域链</strong>和 <strong>this</strong> 是两套不同的系统，它们之间基本没太多联系。在前期明确这点，可以避免你在学习 this 的过程中，和作用域产生一些不必要的关联。</p>
...
<p><img src="/markdown-reader/images/11/b398610fd8060b381d33afc9b86f988d.png" alt="执行上下文中的 this"></p>
<p>从图中可以看出，<strong>this 是和执行上下文绑定的</strong>，也就是说每个执行上下文中都有一个 this。前面<a href="/markdown-reader/guide/08">《08 | 调用栈：为什么 JavaScript 代码会出现栈溢出？》</a>中我们提到过，执行上下文主要分为三种——全局执行上下文、函数执行上下文和 eval 执行上下文，所以对应的 this 也只有这三种——全局执行上下文中的 this、函数中的 this 和 eval 中的 this。</p>
...
<h3 id="1-通过函数的-call-方法设置">1. 通过函数的 call 方法设置</h3>
...
<h3 id="2-通过对象调用方法设置">2. 通过对象调用方法设置</h3>
...
<pre><code class="language-js">myObj.showThis.call(myObj)
</code></pre>
...
<h3 id="3-通过构造函数中设置">3. 通过构造函数中设置</h3>
<p>你可以像这样设置构造函数中的 this，如下面的示例代码：</p>
<pre><code class="language-js">function CreateObj(){
  this.name = &quot;极客时间&quot;
}
...
<h3 id="1-嵌套函数中的-this-不会从外层函数中继承">1. 嵌套函数中的 this 不会从外层函数中继承</h3>
...
<p>执行这段代码，你可以看到它输出了我们想要的结果，最终 myObj 中的 name 属性值变成了“极客邦”。其实，这个方法的的本质是把 <strong>this 体系转换为了作用域的体系</strong>。</p>
...
<ul>
<li><p>第一种是把 this 保存为一个 self 变量，再利用变量的作用域机制传递给嵌套函数。</p>
</li>
<li><p>第二种是继续使用 this，但是要把嵌套函数改为箭头函数，因为箭头函数没有自己的执行上下文，所以它会继承调用函数中的 this。</p>
</li>
</ul>
<h3 id="2-普通函数中的-this-默认指向全局对象-window">2. 普通函数中的 this 默认指向全局对象 window</h3>
...
<ol>
<li><p>当函数作为对象的方法调用时，函数中的 this 就是该对象；</p>
</li>
...
<h2 id="思考时间">思考时间</h2>
<p>你可以观察下面这段代码：</p>
<pre><code class="language-js">let userInfo = {
  name:&quot;jack.ma&quot;,
  age:13,
  sex:male,
  updateInfo:function(){
...
<p>我想通过 updateInfo 来更新 userInfo 里面的数据信息，但是这段代码存在一些问题，你能修复这段代码吗？</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/11"};export{n as default};
