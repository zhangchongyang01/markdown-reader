const n={id:"20",title:"20 | async/await：使用同步的方式去写异步代码",content:`<h1 id="20--asyncawait：使用同步的方式去写异步代码">20 | async/await：使用同步的方式去写异步代码</h1>
<p>在<a href="/markdown-reader/guide/19">上篇文章</a>中，我们介绍了怎么使用 Promise 来实现回调操作，使用 Promise 能很好地解决回调地狱的问题，但是这种方式充满了 Promise 的 then() 方法，如果处理流程比较复杂的话，那么整段代码将充斥着 then，语义化不明显，代码不能很好地表示执行流程。</p>
...
<pre><code class="language-js">fetch(&#39;https://www.geekbang.org&#39;)
      .then((response) =&gt; {
...
<p>从这段 Promise 代码可以看出来，使用 promise.then 也是相当复杂，虽然整个请求流程已经线性化了，但是代码里面包含了大量的 then 函数，使得代码依然不是太容易阅读。<strong>基于这个原因，ES7 引入了 async/await，这是 JavaScript 异步编程的一个重大改进，提供了在不阻塞主线程的情况下使用同步代码实现异步访问资源的能力，并且使得代码逻辑更加清晰。</strong>你可以参考下面这段代码：</p>
...
<h2 id="生成器-vs-协程">生成器 VS 协程</h2>
<p>我们先来看看什么是生成器函数？</p>
<p><strong>生成器函数是一个带星号函数，而且是可以暂停执行和恢复执行的</strong>。我们可以看下面这段代码：</p>
<pre><code class="language-js">function* genDemo() {
    console.log(&quot;开始执行第一段&quot;)
    yield &#39;generator 2&#39;
...
<ol>
<li><p>外部函数可以通过 next 方法恢复函数的执行。</p>
</li>
<li><p>关于函数的暂停和恢复，相信你一定很好奇这其中的原理，那么接下来我们就来简单介绍下 JavaScript 引擎 V8 是如何实现一个函数的暂停和恢复的，这也会有助于你理解后面要介绍的 async/await。</p>
</li>
</ol>
...
<p><img src="/markdown-reader/images/20/5ef98bd693bcd5645e83418b0856e437.png" alt="协程执行流程图"></p>
<p>从图中可以看出来协程的四点规则：</p>
<ol>
<li><p>通过调用生成器函数 genDemo 来创建一个协程 gen，创建之后，gen 协程并没有立即执行。</p>
</li>
<li><p>要让 gen 协程执行，需要通过调用 gen.next。</p>
</li>
<li><p>当协程正在执行的时候，可以通过 yield 关键字来暂停 gen 协程的执行，并返回主要信息给父协程。</p>
</li>
<li><p>如果协程在执行期间，遇到了 return 关键字，那么 JavaScript 引擎会结束当前协程，并将 return 后面的内容返回给父协程。</p>
</li>
</ol>
...
<p><img src="/markdown-reader/images/20/925f4a9a1c85374352ee93c5e3c41440.png" alt="gen 协程和父协程之间的切换"></p>
...
<pre><code class="language-js">//foo函数
function* foo() {
...
<ul>
<li><p>首先执行的是let gen = foo()，创建了 gen 协程。</p>
</li>
<li><p>然后在父协程中通过执行 gen.next 把主线程的控制权交给 gen 协程。</p>
</li>
</ul>
<p>g- en 协程获取到主线程的控制权后，就调用 fetch 函数创建了一个 Promise 对象 response1，然后通过 yield 暂停 gen 协程的执行，并将 response1 返回给父协程。</p>
<ul>
<li><p>父协程恢复执行后，调用 response1.then 方法等待请求结果。</p>
</li>
<li><p>等通过 fetch 发起的请求完成之后，会调用 then 中的回调函数，then 中的回调函数拿到结果之后，通过调用 gen.next 放弃主线程的控制权，将控制权交 gen 协程继续执行下个请求。</p>
</li>
</ul>
...
<pre><code class="language-js">Promise {&lt;resolved&gt;: 2}
</code></pre>
...
<p><img src="/markdown-reader/images/20/8dcd8cfa77d43d1fb928d8b001229b94.png" alt="async/await 执行流程图"></p>
...
<pre><code class="language-js">let promise_ = new Promise((resolve,reject){
  resolve(100)
})
</code></pre>
<p>在这个 promise_ 对象创建的过程中，我们可以看到在 executor 函数中调用了 resolve 函数，JavaScript 引擎会将该任务提交给微任务队列（<a href="/markdown-reader/guide/19">上一篇文章</a>中我们讲解过）。</p>
...
<pre><code class="language-js">promise_.then((value)=&gt;{
   //回调函数被激活后
...
`,path:"/guide/20"};export{n as default};
