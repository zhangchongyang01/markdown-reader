const n={id:"03",title:"03 | HTTP请求流程：为什么很多站点第二次打开速度会很快？",content:`<h1 id="03--http请求流程：为什么很多站点第二次打开速度会很快？">03 | HTTP请求流程：为什么很多站点第二次打开速度会很快？</h1>
<p>在<a href="/markdown-reader/guide/01">上一篇文章</a>中我介绍了 TCP 协议是如何保证数据完整传输的，相信你还记得，一个 TCP 连接过程包括了建立连接、传输数据和断开连接三个阶段。</p>
...
<h2 id="浏览器端发起-http-请求流程">浏览器端发起 HTTP 请求流程</h2>
<p>如果你在浏览器地址栏里键入极客时间网站的地址：<a href="http://time.geekbang.org/index.html%EF%BC%8C">http://time.geekbang.org/index.html，</a> 那么接下来，浏览器会完成哪些动作呢？下面我们就一步一步详细“追踪”下。</p>
<h3 id="1-构建请求">1. 构建请求</h3>
<p>首先，浏览器构建请求行信息（如下所示），构建好后，浏览器准备发起网络请求。</p>
<pre><code>GET /index.html HTTP1.1
</code></pre>
<h3 id="2-查找缓存">2. 查找缓存</h3>
...
<h3 id="3-准备-ip-地址和端口">3. 准备 IP 地址和端口</h3>
...
<p><img src="/markdown-reader/images/03/1277f342174b23f9442d3b27016d7980.png" alt="img"></p>
...
<h3 id="4-等待-tcp-队列">4. 等待 TCP 队列</h3>
...
<h3 id="1-返回请求">1. 返回请求</h3>
<p>一旦服务器处理结束，便可以返回数据给浏览器了。你可以通过工具软件 curl 来查看返回请求数据，具体使用方法是在命令行中输入以下命令：</p>
<pre><code>curl -i  https://time.geekbang.org/
</code></pre>
<p>注意这里加上了-i是为了返回响应行、响应头和响应体的数据，返回的结果如下图所示，你可以结合这些数据来理解服务器是如何响应浏览器的。</p>
<p><img src="/markdown-reader/images/03/3e30476a4bbda49fd7cd4fd0ea09f076.png" alt="img"></p>
...
<p>到这里似乎请求流程快结束了，不过还有一种情况你需要了解下，比如当你在浏览器中打开 geekbang.org 后，你会发现最终打开的页面地址是 <a href="https://www.geekbang.org%E3%80%82">https://www.geekbang.org。</a></p>
<p>这两个 URL 之所以不一样，是因为涉及到了一个重定向操作。跟前面一样，你依然可以使用 curl 来查看下请求 geekbang.org 会返回什么内容？</p>
<p>在控制台输入如下命令：</p>
<pre><code>curl -I geekbang.org
</code></pre>
<p>注意这里输入的参数是-I，和-i不一样，-I表示只需要获取响应头和响应行数据，而不需要获取响应体的数据，最终返回的数据如下图所示：</p>
<p><img src="/markdown-reader/images/03/28d5796c6ab7faa619ed8f1bd17b0843.jpg" alt="img"></p>
<p>从图中你可以看到，响应行返回的状态码是 301，状态 301 就是告诉浏览器，我需要重定向到另外一个网址，而需要重定向的网址正是包含在响应头的 Location 字段中，接下来，浏览器获取 Location 字段中的地址，并使用该地址重新导航，这就是一个完整重定向的执行流程。这也就解释了为什么输入的是 geekbang.org，最终打开的却是 <a href="https://www.geekbang.org">https://www.geekbang.org</a> 了。</p>
<p>不过也不要认为这种跳转是必然的。如果你打开 <a href="https://12306.cn%EF%BC%8C%E4%BD%A0%E4%BC%9A%E5%8F%91%E7%8E%B0%E8%BF%99%E4%B8%AA%E7%AB%99%E7%82%B9%E6%98%AF%E6%89%93%E4%B8%8D%E5%BC%80%E7%9A%84%E3%80%82%E8%BF%99%E6%98%AF%E5%9B%A0%E4%B8%BA">https://12306.cn，你会发现这个站点是打不开的。这是因为</a> 12306 的服务器并没有处理跳转，所以必须要手动输入完整的 <a href="https://www.12306.cn">https://www.12306.cn</a> 才能打开页面。</p>
<h2 id="问题解答">问题解答</h2>
<p>说了这么多，相信你现在已经了解了 HTTP 的请求流程，那现在我们再回过头来看看文章开头提出的问题。</p>
<h3 id="1-为什么很多站点第二次打开速度会很快？">1. 为什么很多站点第二次打开速度会很快？</h3>
...
<p><img src="/markdown-reader/images/03/5fc2f88a04ee0fc41a808f3481287408.png" alt="img"></p>
...
<p>关于缓存的细节内容特别多，具体细节你可以参考这篇 <a href="https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching">HTTP 缓存</a>，在这里我就不赘述了。</p>
<p>简要来说，很多网站第二次访问能够秒开，是因为这些网站把很多资源都缓存在了本地，浏览器缓存直接使用本地副本来回应请求，而不会产生真实的网络请求，从而节省了时间。同时，DNS 数据也被浏览器缓存了，这又省去了 DNS 查询环节。</p>
<h3 id="2-登录状态是如何保持的？">2. 登录状态是如何保持的？</h3>
...
<pre><code>Set-Cookie: UID=3431uad;
</code></pre>
<ul>
<li>浏览器在接收到服务器的响应头后，开始解析响应头，如果遇到响应头里含有 Set-Cookie 字段的情况，浏览器就会把这个字段信息保存到本地。比如把UID=3431uad保持到本地。</li>
<li>当用户再次访问时，浏览器会发起 HTTP 请求，但在发起请求之前，浏览器会读取之前保存的 Cookie 数据，并把数据写进请求头里的 Cookie 字段里（如下所示），然后浏览器再将请求头发送给服务器。<br>Cookie: UID=3431uad;</li>
...
<p><img src="/markdown-reader/images/03/d9d6cefe8d3d6d84a37a626687c6ecb3.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/03/1b49976aca2c700883d48d927f48986c.png" alt="img"></p>
...
`,path:"/guide/03"};export{n as default};
