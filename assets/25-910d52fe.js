const n={id:"25",title:"25 | 页面性能：如何系统地优化页面？",content:`<h1 id="25--页面性能：如何系统地优化页面？">25 | 页面性能：如何系统地优化页面？</h1>
...
<ul>
<li><p>加载阶段，是指从发出请求到渲染出完整页面的过程，影响到这个阶段的主要因素有网络和 JavaScript 脚本。</p>
</li>
<li><p>交互阶段，主要是从页面加载完成到用户交互的整合过程，影响到这个阶段的主要因素是 JavaScript 脚本。</p>
</li>
<li><p>关闭阶段，主要是用户发出关闭指令后页面所做的一些清理操作。</p>
</li>
</ul>
<p>这里我们需要重点关注<strong>加载阶段和交互阶段</strong>，因为影响到我们体验的因素主要都在这两个阶段，下面我们就来逐个详细分析下。</p>
<h2 id="加载阶段">加载阶段</h2>
<p>我们先来分析如何系统优化加载阶段中的页面，还是先看一个典型的渲染流水线，如下图所示：</p>
<p><img src="/markdown-reader/images/25/5d8716586b5f4d719097dca881007a7b.jpg" alt="加载阶段渲染流水线"></p>
...
<p><strong>第三个是请求关键资源需要多少个 RTT（Round Trip Time）</strong>。那什么是 RTT 呢？ 在<a href="/markdown-reader/guide/02">《02 | TCP 协议：如何保证页面文件能被完整送达浏览器？》</a>这篇文章中我们分析过，当使用 TCP 协议传输一个文件时，比如这个文件大小是 0.1M，由于 TCP 的特性，这个数据并不是一次传输到服务端的，而是需要拆分成一个个数据包来回多次进行传输的。<strong>RTT 就是这里的往返时延。它是网络中一个重要的性能指标，表示从发送端发送数据开始，到发送端收到来自接收端的确认，总共经历的时延</strong>。通常 1 个 HTTP 的数据包在 14KB 左右，所以 1 个 0.1M 的页面就需要拆分成 8 个包来传输了，也就是说需要 8 个 RTT。</p>
...
<ul>
<li><p>如何减少关键资源的个数？一种方式是可以将 JavaScript 和 CSS 改成内联的形式，比如上图的 JavaScript 和 CSS，若都改成内联模式，那么关键资源的个数就由 3 个减少到了 1 个。另一种方式，如果 JavaScript 代码没有 DOM 或者 CSSOM 的操作，则可以改成 async 或者 defer 属性；同样对于 CSS，如果不是在构建页面之前加载的，则可以添加媒体取消阻止显现的标志。当 JavaScript 标签加上了 async 或者 defer、CSSlink 属性之前加上了取消阻止显现的标志后，它们就变成了非关键资源了。</p>
</li>
<li><p>如何减少关键资源的大小？可以压缩 CSS 和 JavaScript 资源，移除 HTML、CSS、JavaScript 文件中一些注释内容，也可以通过前面讲的取消 CSS 或者 JavaScript 中关键资源的方式。</p>
</li>
<li><p>如何减少关键资源 RTT 的次数？可以通过减少关键资源的个数和减少关键资源的大小搭配来实现。除此之外，还可以使用 CDN 来减少每次 RTT 时长。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/25/4a942e53f9358c9c4634c310335cc10c.png" alt="交互阶段渲染流水线"></p>
...
<h2 id="1-减少-javascript-脚本执行时间">1. 减少 JavaScript 脚本执行时间</h2>
<p>有时 JavaScript 函数的一次执行时间可能有几百毫秒，这就严重霸占了主线程执行其他渲染任务的时间。针对这种情况我们可以采用以下两种策略：</p>
<ul>
<li><p>一种是将一次执行的函数分解为多个任务，使得每次的执行时间不要过久。</p>
</li>
<li><p>另一种是采用 Web Workers。你可以把 Web Workers 当作主线程之外的一个线程，在 Web Workers 中是可以执行 JavaScript 脚本的，不过 Web Workers 中没有 DOM、CSSOM 环境，这意味着在 Web Workers 中是无法通过 JavaScript 来访问 DOM 的，所以我们可以把一些和 DOM 操作无关且耗时的任务放到 Web Workers 中去执行。</p>
</li>
</ul>
<p>总之，在交互阶段，对 JavaScript 脚本总的原则就是不要一次霸占太久主线程。</p>
<h2 id="2-避免强制同步布局">2. 避免强制同步布局</h2>
...
<p><img src="/markdown-reader/images/25/32b6a645646f99fc3517fb0b5e003cc9.png" alt="Performance 记录添加元素的执行过程"></p>
...
<p><img src="/markdown-reader/images/25/ce951be7a38e2ef1a9a23a1c7e84b1d9.png" alt="触发强制同步布局 Performance 图"></p>
...
<p><img src="/markdown-reader/images/25/36159f7081e37ce4714b20ce2630e987.png" alt="Performance 中关于布局抖动的表现"></p>
<p>从上图可以看出，在 foo 函数内部重复执行计算样式和布局，这会大大影响当前函数的执行效率。这种情况的避免方式和强制同步布局一样，都是尽量不要在修改 DOM 结构时再去查询一些相关值。</p>
<h2 id="4-合理利用-css-合成动画">4. 合理利用 CSS 合成动画</h2>
...
<h2 id="5-避免频繁的垃圾回收">5. 避免频繁的垃圾回收</h2>
...
<h2 id="思考时间">思考时间</h2>
<p>那你来分析下新浪官网（<a href="https://www.sina.com.cn/">https://www.sina.com.cn/</a> ）在加载阶段和交互阶段所存在的一些性能问题。</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/25"};export{n as default};
