const t={id:"22",title:"22 | DOM树：JavaScript是如何影响DOM树构建的？",content:`<h1 id="22--dom树：javascript是如何影响dom树构建的？">22 | DOM树：JavaScript是如何影响DOM树构建的？</h1>
<p>在<a href="/markdown-reader/guide/21">上一篇文章</a>中，我们通过开发者工具中的网络面板，介绍了网络请求过程的几种<strong>性能指标</strong>以及对页面加载的影响。</p>
...
<ul>
<li><p>从页面的视角来看，DOM 是生成页面的基础数据结构。</p>
</li>
<li><p>从 JavaScript 脚本视角来看，DOM 提供给 JavaScript 脚本操作的接口，通过这套接口，JavaScript 可以对 DOM 结构进行访问，从而改变文档的结构、样式和内容。</p>
</li>
<li><p>从安全视角来看，DOM 是一道安全防护线，一些不安全的内容在 DOM 解析阶段就被拒之门外了。</p>
</li>
</ul>
<p>简言之，DOM 是表述 HTML 的内部数据结构，它会将 Web 页面和 JavaScript 脚本连接起来，并过滤一些不安全的内容。</p>
<h2 id="dom-树如何生成">DOM 树如何生成</h2>
...
<p><img src="/markdown-reader/images/22/1bfcd419acf6402c20ffc1a5b1909d8c.png" alt="字节流转换为 DOM"></p>
<p>从图中你可以看出，字节流转换为 DOM 需要三个阶段。</p>
<p><strong>第一个阶段，通过分词器将字节流转换为 Token。</strong></p>
<p>前面<a href="/markdown-reader/guide/14">《14 | 编译器和解释器：V8 是如何执行一段 JavaScript 代码的？》</a>文章中我们介绍过，V8 编译 JavaScript 过程中的第一步是做词法分析，将 JavaScript 先分解为一个个 Token。解析 HTML 也是一样的，需要通过分词器先将字节流转换为一个个 Token，分为 Tag Token 和文本 Token。上述 HTML 代码通过词法分析生成的 Token 如下所示：</p>
<p><img src="/markdown-reader/images/22/b16d2fbb77e12e376ac0d7edec20ceac.png" alt="生成的 Token 示意图"></p>
...
<p><img src="/markdown-reader/images/22/7a6cd022bd51a3f274cd994b1398bef1.png" alt="解析到 StartTag html 时的状态"></p>
<p>然后按照同样的流程解析出来 StartTag body 和 StartTag div，其 Token 栈和 DOM 的状态如下图所示：</p>
<p><img src="/markdown-reader/images/22/8c7ba966cebb0050b81c0385ffb4f2a5.png" alt="解析到 StartTag div 时的状态"></p>
<p>接下来解析出来的是第一个 div 的文本 Token，渲染引擎会为该 Token 创建一个文本节点，并将该 Token 添加到 DOM 中，它的父节点就是当前 Token 栈顶元素对应的节点，如下图所示：</p>
<p><img src="/markdown-reader/images/22/dc0ddd4e3bf3569555f4b1ebec7a8caf.png" alt="解析出第一个文本 Token 时的状态"></p>
<p>再接下来，分词器解析出来第一个 EndTag div，这时候 HTML 解析器会去判断当前栈顶的元素是否是 StartTag div，如果是则从栈顶弹出 StartTag div，如下图所示：</p>
<p><img src="/markdown-reader/images/22/c4a255a8881ef9d21e419aa010ce24a6.png" alt="元素弹出 Token 栈示意图"></p>
<p>按照同样的规则，一路解析，最终结果如下图所示：</p>
<p><img src="/markdown-reader/images/22/aabf14cde38b058c5203195db82ec22e.png" alt="最终解析结果"></p>
...
<h2 id="javascript-是如何影响-dom-生成的">JavaScript 是如何影响 DOM 生成的</h2>
...
<p><img src="/markdown-reader/images/22/4150e27b332fab9f5a10bfafb524ff54.png" alt="执行脚本时 DOM 的状态"></p>
...
<p>这段代码的功能还是和前面那段代码是一样的，不过这里我把内嵌 JavaScript 脚本修改成了通过 JavaScript 文件加载。其整个执行流程还是一样的，执行到 JavaScript 标签时，暂停整个 DOM 的解析，执行 JavaScript 代码，不过这里执行 JavaScript 时，需要先下载这段 JavaScript 代码。<strong>这里需要重点关注下载环境，因为 JavaScript 文件的下载过程会阻塞 DOM 解析</strong>，而通常下载又是非常耗时的，会受到网络环境、JavaScript 文件大小等因素的影响。</p>
...
<pre><code class="language-html">&lt;script async type=&quot;text/javascript&quot; src=&#39;foo.js&#39;&gt;&lt;/script&gt;
</code></pre>
<pre><code class="language-html">&lt;script defer type=&quot;text/javascript&quot; src=&#39;foo.js&#39;&gt;&lt;/script&gt;
</code></pre>
...
`,path:"/guide/22"};export{t as default};
