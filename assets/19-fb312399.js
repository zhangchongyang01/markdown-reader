const e={id:"19",title:"19 | Promise：使用Promise，告别回调函数",content:`<h1 id="19--promise：使用promise，告别回调函数">19 | Promise：使用Promise，告别回调函数</h1>
<p>在<a href="/markdown-reader/guide/18">上一篇文章</a>中我们聊到了微任务是如何工作的，并介绍了 MutationObserver 是如何利用微任务来权衡性能和效率的。今天我们就接着来聊聊微任务的另外一个应用 <strong>Promise</strong>，DOM/BOM API 中新加入的 API 大多数都是建立在 Promise 上的，而且新的前端框架也使用了大量的 Promise。可以这么说，Promise 已经成为现代前端的“水”和“电”，很是关键，所以深入学习 Promise 势在必行。</p>
...
<h2 id="异步编程的问题：代码逻辑不连续">异步编程的问题：代码逻辑不连续</h2>
...
<p><img src="/markdown-reader/images/19/01e40e30db7e8a91eb70ce02fd8a6985.png" alt="Web 应用的异步编程模型"></p>
...
<pre><code class="language-js">//执行状态
...
<h2 id="封装异步代码，让处理流程变得线性">封装异步代码，让处理流程变得线性</h2>
<p>由于我们重点关注的是<strong>输入内容（请求信息）和输出内容（回复信息）</strong>，至于中间的异步请求过程，我们不想在代码里面体现太多，因为这会干扰核心的代码逻辑。整体思路如下图所示：</p>
<p><img src="/markdown-reader/images/19/83dd5231c2e36c636c61af6a6dc80a5c.png" alt="封装请求过程"></p>
...
<pre><code class="language-js">//makeRequest用来构造request对象
...
<p>这段代码是先请求time.geekbang.org/?category，如果请求成功的话，那么再请求time.geekbang.org/column，如果再次请求成功的话，就继续请求time.geekbang.org。也就是说这段代码用了三层嵌套请求，就已经让代码变得混乱不堪，所以，我们还需要解决这种嵌套调用后混乱的代码结构。</p>
<p>这段代码之所以看上去很乱，归结其原因有两点：</p>
<ul>
<li><p><strong>第一是嵌套调用</strong>，下面的任务依赖上个任务的请求结果，并在上个任务的回调函数内部执行新的业务逻辑，这样当嵌套层次多了之后，代码的可读性就变得非常差了。</p>
</li>
<li><p><strong>第二是任务的不确定性</strong>，执行每个任务都有两种可能的结果（成功或者失败），所以体现在代码中就需要对每个任务的执行结果做两次判断，这种对每个任务都要进行一次额外的错误处理的方式，明显增加了代码的混乱程度。</p>
</li>
</ul>
...
<ul>
<li><p>首先我们引入了 Promise，在调用 XFetch 时，会返回一个 Promise 对象。</p>
</li>
<li><p>构建 Promise 对象时，需要传入一个 <strong>executor 函数</strong>，XFetch 的主要业务流程都在 executor 函数中执行。</p>
</li>
<li><p>如果运行在 excutor 函数中的业务执行成功了，会调用 resolve 函数；如果执行失败了，则调用 reject 函数。</p>
</li>
<li><p>在 excutor 函数中调用 resolve 函数时，会触发 promise.then 设置的回调函数；而调用 reject 函数时，会触发 promise.catch 设置的回调函数。</p>
</li>
</ul>
...
<pre><code class="language-js">//创建Promise对象x1，并在executor函数中执行业务逻辑
function executor(resolve, reject){
    resolve(100)
}
...
<p><img src="/markdown-reader/images/19/efcc4fcbebe75b4f6e92c89b968b4a7f.png" alt="回调函数返回值穿透到最外层"></p>
...
<h2 id="promise-与微任务">Promise 与微任务</h2>
...
<p>之所以出现这个错误，是由于 Bromise 的延迟绑定导致的，在调用到 onResolve_ 函数的时候，Bromise.then 还没有执行，所以执行上述代码的时候，当然会报“onResolve_ is not a function“的错误了。</p>
...
<ol>
<li><p>Promise 中为什么要引入微任务？</p>
</li>
<li><p>Promise 中是如何实现回调函数返回值穿透的？</p>
</li>
<li><p>Promise 出错后，是怎么通过“冒泡”传递给最后那个捕获异常的函数？</p>
</li>
</ol>
<p>这三个问题你不用急着完成，可以先花一段时间查阅材料，然后再来一道一道解释。搞清楚了这三道题目，你也就搞清楚了 Promise。</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/19"};export{e as default};
