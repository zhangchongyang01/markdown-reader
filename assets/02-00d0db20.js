const n={id:"02",title:"02 | TCP协议：如何保证页面文件能被完整送达浏览器？",content:`<h1 id="02--tcp协议：如何保证页面文件能被完整送达浏览器？">02 | TCP协议：如何保证页面文件能被完整送达浏览器？</h1>
...
<h3 id="1-ip：把数据包送达目的主机">1. IP：把数据包送达目的主机</h3>
...
<p>如果要想把一个数据包从主机 A 发送给主机 B，那么在传输之前，数据包上会被附加上主机 B 的 IP 地址信息，这样在传输过程中才能正确寻址。额外地，数据包上还会附加上主机 A 本身的 IP 地址，有了这些信息主机 B 才可以回复信息给主机 A。这些附加的信息会被装进一个叫 IP 头的数据结构里。IP 头是 IP 数据包开头的信息，包含 IP 版本、源 IP 地址、目标 IP 地址、生存时间等信息。如果你要详细了解 IP 头信息，可以参考<a href="https://zh.wikipedia.org/wiki/%E4%BC%A0%E8%BE%93%E6%8E%A7%E5%88%B6%E5%8D%8F%E8%AE%AE">该链接</a>。</p>
<p>为了方便理解，我先把网络简单分为三层结构，如下图：</p>
<p><img src="/markdown-reader/images/02/00d9bcad0bda1fdb43ead428e89ae74d.png" alt="img"></p>
...
<h3 id="2-udp：把数据包送达应用程序">2. UDP：把数据包送达应用程序</h3>
...
<p><img src="/markdown-reader/images/02/3edb673a43f23d84253c52124ce447ea.png" alt="img"></p>
<p>下面我们一起来看下一个数据包从主机 A 旅行到主机 B 的路线：</p>
<ul>
<li><p>上层将含有“极客时间”的数据包交给传输层；</p>
</li>
<li><p>传输层会在数据包前面附加上 UDP 头，组成新的 UDP 数据包，再将新的 UDP 数据包交给网络层；</p>
</li>
<li><p>网络层再将 IP 头附加到数据包上，组成新的 IP 数据包，并交给底层；</p>
</li>
...
<h3 id="3-tcp：把数据完整地送达应用程序">3. TCP：把数据完整地送达应用程序</h3>
...
<p><img src="/markdown-reader/images/02/943ac29f7d5b45a8861b0cde5da99032.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/02/440ee50de56edc27c6b3c992b3a25844.png" alt="img"></p>
<ul>
<li><p><strong>首先，建立连接阶段</strong>。这个阶段是通过“三次握手”来建立客户端和服务器之间的连接。TCP 提供面向连接的通信传输。<strong>面向连接</strong>是指在数据通信开始之前先做好两端之间的准备工作。所谓<strong>三次握手</strong>，是指在建立一个 TCP 连接时，客户端和服务器总共要发送三个数据包以确认连接的建立。</p>
</li>
<li><p><strong>其次，传输数据阶段</strong>。在该阶段，<strong>接收端需要对每个数据包进行确认操作</strong>，也就是接收端在接收到数据包之后，需要发送确认数据包给发送端。所以当发送端发送了一个数据包之后，在规定时间内没有接收到接收端反馈的确认消息，则判断为数据包丢失，并触发发送端的重发机制。同样，一个大的文件在传输过程中会被拆分成很多小的数据包，这些数据包到达接收端后，接收端会按照 TCP 头中的序号为其排序，从而保证组成完整的数据。</p>
</li>
...
<li><p>而 TCP 保证了数据完整地传输，它的连接可分为三个阶段：建立连接、传输数据和断开连接。</p>
</li>
</ul>
...
`,path:"/guide/02"};export{n as default};
