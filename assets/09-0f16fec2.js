const n={id:"09",title:"09 | 块级作用域：var缺陷以及为什么要引入let和const？",content:`<h1 id="09--块级作用域：var缺陷以及为什么要引入let和const？">09 | 块级作用域：var缺陷以及为什么要引入let和const？</h1>
<p>在前面<a href="/markdown-reader/guide/07">《07 | 变量提升：JavaScript 代码是按顺序执行的吗？》</a>这篇文章中，我们已经讲解了 JavaScript 中变量提升的相关内容，正是<strong>由于 JavaScript 存在变量提升这种特性，从而导致了很多与直觉不符的代码，这也是 JavaScript 的一个重要设计缺陷</strong>。</p>
...
<pre><code class="language-js">//if块
if(1){}
...
<pre><code class="language-c">char* myname = &quot;极客时间&quot;;
void showName() {
  printf(&quot;%s \\n&quot;,myname);
  if(0){
    char* myname = &quot;极客邦&quot;;
...
<p>上面这段 C 代码执行后，最终打印出来的是上面全局变量 myname 的值，之所以这样，是因为 C 语言是支持块级作用域的，所以 if 块里面定义的变量是不能被 if 块外面的语句访问到的。</p>
...
<h3 id="1-变量容易在不被察觉的情况下被覆盖掉">1. 变量容易在不被察觉的情况下被覆盖掉</h3>
...
<p>执行上面这段代码，打印出来的是 undefined，而并没有像前面 C 代码那样打印出来“极客时间”的字符串。为什么输出的内容是 undefined 呢？我们再来分析一下。</p>
<p>首先当刚执行到 showName 函数调用时，执行上下文和调用栈的状态是怎样的？具体分析过程你可以回顾<a href="/markdown-reader/guide/08">《08 | 调用栈：为什么 JavaScript 代码会出现栈溢出？》</a>这篇文章的分析过程，这里我就直接展示出来了，最终的调用栈状态如下图所示：</p>
<p><img src="/markdown-reader/images/09/944aaeaeb9ee50feea3c7d218acdd5c9.png" alt="开始执行 showName 函数时的调用栈"></p>
...
<h3 id="2-本应销毁的变量没有被销毁">2. 本应销毁的变量没有被销毁</h3>
...
<p>如果你使用 C 语言或者其他的大部分语言实现类似代码，在 for 循环结束之后，i 就已经被销毁了，但是在 JavaScript 代码中，i 的值并未被销毁，所以最后打印出来的是 7。</p>
...
<pre><code class="language-js">function varTest() {
  var x = 1;
  if (true) {
    var x = 2; // 同样的变量!
    console.log(x); // 2
...
<p><img src="/markdown-reader/images/09/4501368679083f3a8e1a9e4a8e316dbf.png" alt="varTest 函数的执行上下文"></p>
...
<pre><code class="language-js">function letTest() {
  let x = 1;
  if (true) {
    let x = 2; // 不同的变量
    console.log(x); // 2
...
<p>执行这段代码，其输出结果就和我们的预期是一致的。这是因为 let 关键字是支持块级作用域的，所以在编译阶段，JavaScript 引擎并不会把 if 块中通过 let 声明的变量存放到变量环境中，这也就意味着在 if 块通过 let 声明的关键字，并不会提升到全函数可见。所以在 if 块之内打印出来的值是 2，跳出语块之后，打印出来的值就是 1 了。这种就非常<strong>符合我们的编程习惯了：作用域块内声明的变量不影响块外面的变量</strong>。</p>
...
<p><img src="/markdown-reader/images/09/f9f67f2f53437218baef9dc724bd4c67.png" alt="刚执行时 foo 函数的执行上下文"></p>
<p>通过上图，我们可以得出以下结论：</p>
<ul>
...
<li><p>在函数内部的作用域块，通过 let 声明的变量并没有被存放到词法环境中。</p>
</li>
</ul>
<p>接下来，<strong>第二步继续执行代码</strong>，当执行到代码块里面时，变量环境中 a 的值已经被设置成了 1，词法环境中 b 的值已经被设置成了 2，这时候函数的执行上下文就如下图所示：</p>
<p><img src="/markdown-reader/images/09/7e0f7bc362e0dea21d27dc5fb08d06fa.png" alt="执行 foo 函数内部作用域块时的执行上下文"></p>
...
<p><img src="/markdown-reader/images/09/06c06a756632acb12aa97b3be57bb908.png" alt="变量查找过程"></p>
<p>从上图你可以清晰地看出变量查找流程，不过要完整理解查找变量或者查找函数的流程，就涉及到作用域链了，这个我们会在下篇文章中做详细介绍。</p>
<p>当作用域块执行结束之后，其内部定义的变量就会从词法环境的栈顶弹出，最终执行上下文如下图所示：</p>
<p><img src="/markdown-reader/images/09/d4f99640d62feba4202aa072f6369d28.png" alt="作用域执行完成示意图"></p>
...
<p>你能通过分析词法环境，得出来最终的打印结果吗？</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/09"};export{n as default};
