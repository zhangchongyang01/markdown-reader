const e={id:"16",title:"16 | WebAPI：setTimeout是如何实现的？",content:`<h1 id="16--webapi：settimeout是如何实现的？">16 | WebAPI：setTimeout是如何实现的？</h1>
<p>在<a href="/markdown-reader/guide/15">上一篇文章</a>中我们介绍了页面中的事件和消息队列，知道了<strong>浏览器页面是由消息队列和事件循环系统来驱动的。</strong></p>
...
<h2 id="浏览器怎么实现-settimeout">浏览器怎么实现 setTimeout</h2>
<p>要了解定时器的工作原理，就得先来回顾下之前讲的事件循环系统，我们知道渲染进程中所有运行在主线程上的任务都需要先添加到消息队列，然后事件循环系统再按照顺序执行消息队列中的任务。下面我们来看看那些典型的事件：</p>
<ul>
<li><p>当接收到 HTML 文档数据，渲染引擎就会将“解析 DOM”事件添加到消息队列中，</p>
</li>
<li><p>当用户改变了 Web 页面的窗口大小，渲染引擎就会将“重新布局”的事件添加到消息队列中。</p>
</li>
<li><p>当触发了 JavaScript 引擎垃圾回收机制，渲染引擎会将“垃圾回收”任务添加到消息队列中。</p>
</li>
<li><p>同样，如果要执行一段异步 JavaScript 代码，也是需要将执行任务添加到消息队列中。</p>
</li>
</ul>
...
<p>如果感兴趣，你可以参考<a href="https://source.chromium.org/chromium/chromium/src/+/main:base/task/sequence_manager/task_queue_impl.h">Chromium 中关于队列部分的源码</a>。</p>
<p>源码中延迟执行队列的定义如下所示：</p>
<pre><code class="language-c++">DelayedIncomingQueue delayed_incoming_queue;
</code></pre>
<p>当通过 JavaScript 调用 setTimeout 设置回调函数的时候，渲染进程将会创建一个回调任务，包含了回调函数 showName、当前发起时间、延迟执行时间，其模拟代码如下所示：</p>
<pre><code class="language-c++">struct DelayTask{
  int64 id；
...
<pre><code class="language-c++">delayed_incoming_queue.push(timerTask)；
</code></pre>
<p>现在通过定时器发起的任务就被保存到延迟队列中了，那接下来我们再来看看消息循环系统是怎么触发延迟队列的。</p>
<p>我们可以来完善<a href="/markdown-reader/guide/15">上一篇文章</a>中消息循环的代码，在其中加入执行延迟队列的代码，如下所示：</p>
<pre><code class="language-c++">void ProcessTimerTask(){
...
<p>从上面代码可以看出来，我们添加了一个 <strong>ProcessDelayTask 函数</strong>，该函数是专门用来处理延迟执行任务的。这里我们要重点关注它的执行时机，在上段代码中，处理完消息队列中的一个任务之后，就开始执行 ProcessDelayTask 函数。ProcessDelayTask 函数会根据发起时间和延迟时间计算出到期的任务，然后依次执行这些到期的任务。等到期的任务执行完成之后，再继续下一个循环过程。通过这样的方式，一个完整的定时器就实现了。</p>
<p>设置一个定时器，JavaScript 引擎会返回一个定时器的 ID。那通常情况下，当一个定时器的任务还没有被执行的时候，也是可以取消的，具体方法是调用** clearTimeout 函数**，并传入需要取消的定时器的 ID。如下面代码所示：</p>
<pre><code class="language-js">clearTimeout(timer_id)
</code></pre>
<p>其实浏览器内部实现取消定时器的操作也是非常简单的，就是直接从 delayed_incoming_queue 延迟队列中，通过 ID 查找到对应的任务，然后再将其从队列中删除掉就可以了。</p>
<h2 id="使用-settimeout-的一些注意事项">使用 setTimeout 的一些注意事项</h2>
<p>现在你应该知道在浏览器内部定时器是如何工作的了。不过在使用定时器的过程中，如果你不了解定时器的一些细节，那么很有可能掉进定时器的一些陷阱里。所以接下来，我们就来讲解一下在使用定时器过程中存在的那些陷阱。</p>
<h3 id="1-如果当前任务执行时间过久，会影响定时器任务的执行">1. 如果当前任务执行时间过久，会影响定时器任务的执行</h3>
...
<p><img src="/markdown-reader/images/16/1adf4da8ca4315cfb565e798649bd74f.png" alt="长任务导致定时器被延后执行"></p>
<p>从图中可以看到，执行 foo 函数所消耗的时长是 500 毫秒，这也就意味着通过 setTimeout 设置的任务会被推迟到 500 毫秒以后再去执行，而设置 setTimeout 的回调延迟时间是 0。</p>
<h3 id="2-如果-settimeout-存在嵌套调用，那么系统会设置最短时间间隔为-4-毫秒">2. 如果 setTimeout 存在嵌套调用，那么系统会设置最短时间间隔为 4 毫秒</h3>
<p>也就是说在定时器函数里面嵌套调用定时器，也会延长定时器的执行时间，可以先看下面的这段代码：</p>
<pre><code class="language-js">function cb() { setTimeout(cb, 0); }
setTimeout(cb, 0);
</code></pre>
<p>上述这段代码你有没有看出存在什么问题？</p>
<p>你还是可以通过 Performance 来记录下这段代码的执行过程，如下图所示：</p>
<p><img src="/markdown-reader/images/16/cbb3b2b1ac8eb4752a585df5445412cd.png" alt="循环嵌套调用 setTimeout"></p>
<p>上图中的竖线就是定时器的函数回调过程，从图中可以看出，前面五次调用的时间间隔比较小，嵌套调用超过五次以上，后面每次的调用最小时间间隔是 4 毫秒。之所以出现这样的情况，是<strong>因为在 Chrome 中，定时器被嵌套调用 5 次以上，系统会判断该函数方法被阻塞了，如果定时器的调用时间间隔小于 4 毫秒，那么浏览器会将每次调用的时间间隔设置为 4 毫秒</strong>。下面是<a href="https://source.chromium.org/chromium/chromium/src/+/main:third_party/blink/renderer/core/frame/dom_timer.cc">Chromium 实现 4 毫秒延迟的代码</a>，你可以看下：</p>
<pre><code class="language-c++">static const int kMaxTimerNestingLevel = 5;
...
<pre><code class="language-c++">base::TimeDelta interval_milliseconds =
...
<h3 id="3-未激活的页面，settimeout-执行最小间隔是-1000-毫秒">3. 未激活的页面，setTimeout 执行最小间隔是 1000 毫秒</h3>
...
<h3 id="4-延时执行时间有最大值">4. 延时执行时间有最大值</h3>
...
<h3 id="5-使用-settimeout-设置的回调函数中的-this-不符合直觉">5. 使用 setTimeout 设置的回调函数中的 this 不符合直觉</h3>
...
<pre><code class="language-js">setTimeout(MyObj.showName.bind(MyObj), 1000)
</code></pre>
<h2 id="总结">总结</h2>
...
`,path:"/guide/16"};export{e as default};
