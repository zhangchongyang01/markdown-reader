const n={id:"10",title:"10 | 作用域链和闭包 ：代码中出现相同的变量，JavaScript引擎是如何选择的？",content:`<h1 id="10--作用域链和闭包-：代码中出现相同的变量，javascript引擎是如何选择的？">10 | 作用域链和闭包 ：代码中出现相同的变量，JavaScript引擎是如何选择的？</h1>
<p>在<a href="/markdown-reader/guide/09">上一篇文章</a>中我们讲到了什么是作用域，以及 ES6 是如何通过变量环境和词法环境来同时支持变量提升和块级作用域，在最后我们也提到了如何通过词法环境和变量环境来查找变量，这其中就涉及到<strong>作用域链</strong>的概念。</p>
...
<p>你觉得这段代码中的 bar 函数和 foo 函数打印出来的内容是什么？这就要分析下这两段代码的执行流程。</p>
<p>通过前面几篇文章的学习，想必你已经知道了如何通过执行上下文来分析代码的执行流程了。那么当这段代码执行到 bar 函数内部时，其调用栈的状态图如下所示：</p>
<p><img src="/markdown-reader/images/10/87d8bbc2bb62b03131802fba074146f7.png" alt="执行 bar 函数时的调用栈"></p>
<p>从图中可以看出，全局执行上下文和 foo 函数的执行上下文中都包含变量 myName，那 bar 函数里面 myName 的值到底该选择哪个呢？</p>
<p>也许你的第一反应是按照调用栈的顺序来查找变量，查找方式如下：</p>
<ol>
<li><p>先查找栈顶是否存在 myName 变量，但是这里没有，所以接着往下查找 foo 函数中的变量。</p>
</li>
<li><p>在 foo 函数中查找到了 myName 变量，这时候就使用 foo 函数中的 myName。</p>
</li>
</ol>
...
<p><img src="/markdown-reader/images/10/20a832656434264db47c93e657e346a7.png" alt="带有外部引用的调用栈示意图"></p>
...
<p><img src="/markdown-reader/images/10/216433d2d0c64149a731d84ba1a07739.png" alt="词法作用域"></p>
...
<p>你可以自己先分析下这段代码的执行流程，看看能否分析出来执行结果。</p>
<p>要想得出其执行结果，那接下来我们就得站在作用域链和词法环境的角度来分析下其执行过程。</p>
<p>在<a href="/markdown-reader/guide/09">上篇文章</a>中我们已经介绍过了，ES6 是支持块级作用域的，当执行到代码块时，如果代码块中有 let 或者 const 声明的变量，那么变量就会存放到该函数的词法环境中。对于上面这段代码，当执行到 bar 函数内部的 if 语句块时，其调用栈的情况如下图所示：</p>
<p><img src="/markdown-reader/images/10/25053af5ae30c8be991fa14631cde0a7.png" alt="块级作用域中是如何查找变量的"></p>
...
<p><img src="/markdown-reader/images/10/d5587b76427a56c5f0b0571e4264b7ef.png" alt="执行到 return bar 时候的调用栈"></p>
...
<p><img src="/markdown-reader/images/10/ee7c1ca481875ad4bdeb4383bd1f883f.png" alt="闭包的产生过程"></p>
...
<p><img src="/markdown-reader/images/10/50e4ba60fc7e420e83b35b95e379b246.png" alt="执行 bar 时调用栈状态"></p>
...
<p><img src="/markdown-reader/images/10/40b8840480a5df4f43ad5f4e7907e3a8.png" alt="开发者工具中的闭包展示"></p>
...
<ul>
<li><p>首先，介绍了什么是作用域链，我们把通过作用域查找变量的链条称为作用域链；作用域链是通过词法作用域来确定的，而词法作用域反映了代码的结构。</p>
</li>
<li><p>其次，介绍了在块级作用域中是如何通过作用域链来查找变量的。</p>
</li>
<li><p>最后，又基于作用域链和词法环境介绍了到底什么是闭包。</p>
</li>
</ul>
...
<p>在上面这段代码中有三个地方定义了 myName，分析这段代码，你觉得这段代码在执行过程中会产生闭包吗？最终打印的结果是什么？</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/10"};export{n as default};
