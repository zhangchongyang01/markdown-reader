const n={id:"extra-6",title:"加餐六｜HTTPS：浏览器如何验证数字证书？",content:`<h1 id="加餐六｜https：浏览器如何验证数字证书？">加餐六｜HTTPS：浏览器如何验证数字证书？</h1>
<p>你好，我是李兵。</p>
<p>在<a href="/markdown-reader/guide/36">《36｜HTTPS：让数据传输更安全》</a>这篇文章中，我们聊了下面几个问题：</p>
<ul>
<li><p>HTTPS 使用了对称和非对称的混合加密方式，这解决了数据传输安全的问题；</p>
</li>
<li><p>HTTPS 引入了中间机构 CA，CA 通过给服务器颁发数字证书，解决了浏览器对服务器的信任问题；</p>
</li>
<li><p>服务器向 CA 机构申请证书的流程；</p>
</li>
<li><p>浏览器验证服务器数字证书的流程。</p>
</li>
</ul>
...
<p>大致流程你也可以参考下图：</p>
<p><img src="/markdown-reader/images/extra-6/f569c80f8f4b25b3bf384037813cdca6.png" alt="数字证书申请过程"></p>
<h2 id="浏览器验证证书的流程">浏览器验证证书的流程</h2>
...
<ul>
<li><p>首先，浏览器利用证书的原始信息计算出信息摘要；</p>
</li>
<li><p>然后，利用 <strong>CA 的公钥</strong>来解密数字证书中的<strong>数字签名</strong>，解密出来的数据也是信息摘要；</p>
</li>
<li><p>最后，判断这两个信息摘要是否相等就可以了。</p>
</li>
</ul>
<p><img src="/markdown-reader/images/extra-6/ae7dbe9f8785441721deb1f7b316f708.png" alt="img"></p>
<p>通过这种方式就验证了数字证书是否是由 CA 机构所签发的，不过这种方式又带来了一个新的疑问：<strong>浏览器是怎么获取到 CA 公钥的？</strong></p>
<h2 id="浏览器是怎么获取到-ca-公钥的？">浏览器是怎么获取到 CA 公钥的？</h2>
<p>通常，当你部署 HTTP 服务器的时候，除了部署当前的数字证书之外，还需要部署 CA 机构的数字证书，CA 机构的数字证书包括了 CA 的公钥，以及 CA 机构的一些基础信息。</p>
<p>因此，极客时间服务器就有了两个数字证书:</p>
<ul>
<li><p>给极客时间域名的数字证书；</p>
</li>
<li><p>给极客时间签名的 CA 机构的数字证书。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/extra-6/cb150e316f4847c71288a8df50bfebd3.png" alt="CA 证书"></p>
...
<h2 id="证明-ca-机构的合法性">证明 CA 机构的合法性</h2>
<p>这里并没有一个非常好的方法来证明 CA 的合法性，妥协的方案是，直接在操作系统中内置这些 CA 机构的数字证书，如下图所示：</p>
<p><img src="/markdown-reader/images/extra-6/43a732eb2ba47d06fbef20c515bd990b.png" alt="操作系统内部内置 CA 数字证书"></p>
...
<h2 id="数字证书链">数字证书链</h2>
<p>于是人们又想出来一个折中的方案，将颁发证书的机构划分为两种类型，**根 CA(Root CAs)和中间 CA(Intermediates CAs)**，通常申请者都是向中间 CA 去申请证书的，而根 CA 作用就是给中间 CA 做认证，一个根 CA 会认证很多中间的 CA，而这些中间 CA 又可以去认证其他的中间 CA。</p>
...
<p><img src="/markdown-reader/images/extra-6/10616d8fc323d33bdecb09b503551cb7.png" alt="数字证书链"></p>
...
<h2 id="如何验证根证书的合法性">如何验证根证书的合法性</h2>
...
`,path:"/guide/extra-6"};export{n as default};
