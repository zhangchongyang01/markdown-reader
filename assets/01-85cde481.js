const n={id:"01",title:"01 | Chrome架构：仅仅打开了1个页面，为什么有4个进程？",content:`<h1 id="01--chrome架构：仅仅打开了1个页面，为什么有4个进程？">01 | Chrome架构：仅仅打开了1个页面，为什么有4个进程？</h1>
...
<p>因此，<strong>学习浏览器的多进程架构是很有必要的</strong>。需要说明的是<strong>，在本专栏中，我所有的分析都是基于 Chrome 浏览器的</strong>。那么多浏览器，为什么偏偏选择 Chrome 浏览器呢？因为 Chrome、微软的 Edge 以及国内的大部分主流浏览器，都是基于 Chromium 二次开发而来；而 Chrome 是 Google 的官方发行版，特性和 Chromium 基本一样，只存在一些产品层面差异；再加上 Chrome 是目前世界上使用率最高的浏览器，所以<strong>Chrome 最具代表性</strong>。</p>
<p>在开始之前，我们一起看下，Chrome 打开一个页面需要启动多少进程？你可以点击 Chrome 浏览器右上角的“选项”菜单，选择“更多工具”子菜单，点击“任务管理器”，这将打开 Chrome 的任务管理器的窗口，如下图：</p>
<p><img src="/markdown-reader/images/01/ce7f8cfe212bec0f53360422e3b03a9e.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/01/3380f0a16c323deda5d3a300804b95da.png" alt="img"></p>
...
<p><strong>2. 线程之间共享进程中的数据。</strong><br>如下图所示，线程之间可以对进程的公共数据进行读写操作。</p>
<p><img src="/markdown-reader/images/01/d0efacd7f299ed99e776cb97da2a799e.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/01/6ddad2419b049b0eb2a8036f3dfff1ca.png" alt="img"></p>
<p>如此多的功能模块运行在一个进程里，是导致单进程浏览器<strong>不稳定、不流畅和不安全</strong>的一个主要因素。下面我就来一一分析下出现这些问题的原因。</p>
<h3 id="问题-1：不稳定">问题 1：不稳定</h3>
...
<h3 id="问题-2：不流畅">问题 2：不流畅</h3>
<p>从上面的“单进程浏览器架构示意图”可以看出，所有页面的渲染模块、JavaScript 执行环境以及插件都是运行在同一个线程中的，这就意味着同一时刻只能有一个模块可以执行。<br>比如，下面这个无限循环的脚本：</p>
...
<h3 id="问题-3：不安全">问题 3：不安全</h3>
<p>这里依然可以从插件和页面脚本两个方面来解释该原因。</p>
<p>插件可以使用 C/C++ 等代码编写，通过插件可以获取到操作系统的任意资源，当你在页面运行一个插件时也就意味着这个插件能完全操作你的电脑。如果是个恶意插件，那么它就可以释放病毒、窃取你的账号密码，引发安全性问题。<br>至于页面脚本，它可以通过浏览器的漏洞来获取系统权限，这些脚本获取系统权限之后也可以对你的电脑做一些恶意的事情，同样也会引发安全问题。</p>
...
<h2 id="多进程浏览器时代">多进程浏览器时代</h2>
<p>好在现代浏览器已经解决了这些问题，是如何解决的呢？这就得聊聊我们这个“多进程浏览器时代”了。</p>
<h2 id="早期多进程架构">早期多进程架构</h2>
<p>你可以先看看下面这张图，这是 2008 年 Chrome 发布时的进程架构。</p>
<p><img src="/markdown-reader/images/01/cdc9215e6c6377fc965b7fac8c3ec960.png" alt="img"></p>
...
<h2 id="目前多进程架构">目前多进程架构</h2>
<p>不过 Chrome 的发展是滚滚向前的，相较之前，目前的架构又有了很多新的变化。我们先看看最新的 Chrome 进程架构，你可以参考下图：</p>
<p><img src="/markdown-reader/images/01/b61cab529fa31301bde290813b4587fc.png" alt="img"></p>
...
<li>更复杂的体系架构。浏览器各模块之间耦合性高、扩展性差等问题，会导致现在的架构已经很难适应新的需求了。<br>对于上面这两个问题，Chrome 团队一直在寻求一种弹性方案，既可以解决资源占用高的问题，也可以解决复杂的体系架构的问题。</li>
</ul>
<h2 id="未来面向服务的架构">未来面向服务的架构</h2>
...
<p><img src="/markdown-reader/images/01/329658fe821252db47b0964037a1de2a.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/01/a9ba86d7b03263fa3997d3733d958176.png" alt="img"></p>
<h2 id="总结">总结</h2>
...
`,path:"/guide/01"};export{n as default};
