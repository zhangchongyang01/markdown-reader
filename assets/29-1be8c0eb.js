const T={id:"29",title:"29 | HTTP/1：HTTP性能优化",content:`<h1 id="29--http1：http性能优化">29 | HTTP/1：HTTP性能优化</h1>
...
<h2 id="超文本传输协议-http09">超文本传输协议 HTTP/0.9</h2>
...
<ul>
<li><p>因为 HTTP 都是基于 TCP 协议的，所以客户端先要根据 IP 地址、端口和服务器建立 TCP 连接，而建立连接的过程就是 TCP 协议三次握手的过程。</p>
</li>
<li><p>建立好连接之后，会发送一个 GET 请求行的信息，如GET /index.html用来获取 index.html。</p>
</li>
<li><p>服务器接收请求信息之后，读取对应的 HTML 文件，并将数据以 ASCII 字符流返回给客户端。</p>
</li>
<li><p>HTML 文档传输完成后，断开连接。</p>
</li>
</ul>
<p><img src="/markdown-reader/images/29/db1166c68c22a45c9858e88a234f1a34.png" alt="HTTP/0.9 请求流程"></p>
<p>总的来说，当时的需求很简单，就是用来传输体积很小的 HTML 文件，所以 HTTP/0.9 的实现有以下三个特点。</p>
<ul>
<li><p>第一个是只有一个请求行，并没有 HTTP 请求头和请求体，因为只需要一个请求行就可以完整表达客户端的需求了。</p>
</li>
...
<h2 id="被浏览器推动的-http10">被浏览器推动的 HTTP/1.0</h2>
...
<p><img src="/markdown-reader/images/29/b52b0d1a26ff2b8607c08e5c50ae687d.png" alt="HTTP/1.0 的请求流程"></p>
<p>有了请求头和响应头，浏览器和服务器就能进行更加深入的交流了。</p>
<p><strong>那 HTTP/1.0 是怎么通过请求头和响应头来支持多种不同类型的数据呢？</strong></p>
<p>要支持多种类型的文件，我们就需要解决以下几个问题。</p>
<ul>
<li><p>首先，浏览器需要知道服务器返回的数据是什么类型的，然后浏览器才能根据不同的数据类型做针对性的处理。</p>
</li>
<li><p>其次，由于万维网所支持的应用变得越来越广，所以单个文件的数据量也变得越来越大。为了减轻传输性能，服务器会对数据进行压缩后再传输，所以浏览器需要知道服务器压缩的方法。</p>
</li>
<li><p>再次，由于万维网是支持全球范围的，所以需要提供国际化的支持，服务器需要对不同的地区提供不同的语言版本，这就需要浏览器告诉服务器它想要什么语言版本的页面。</p>
</li>
<li><p>最后，由于增加了各种不同类型的文件，而每种文件的编码形式又可能不一样，为了能够准确地读取文件，浏览器需要知道文件的编码类型。</p>
</li>
</ul>
...
<ul>
<li><p>有的请求服务器可能无法处理，或者处理出错，这时候就需要告诉浏览器服务器最终处理该请求的情况，这就引入了<strong>状态码</strong>。状态码是通过响应行的方式来通知浏览器的。</p>
</li>
<li><p>为了减轻服务器的压力，在 HTTP/1.0 中提供了 <strong>Cache 机制</strong>，用来缓存已经下载过的数据。</p>
</li>
<li><p>服务器需要统计客户端的基础信息，比如 Windows 和 macOS 的用户数量分别是多少，所以 HTTP/1.0 的请求头中还加入了用户代理的字段。</p>
</li>
</ul>
<h2 id="缝缝补补的-http11">缝缝补补的 HTTP/1.1</h2>
...
<h3 id="1-改进持久连接">1. 改进持久连接</h3>
<p>HTTP/1.0 每进行一次 HTTP 通信，都需要经历建立 TCP 连接、传输 HTTP 数据和断开 TCP 连接三个阶段（如下图）。</p>
<p><img src="/markdown-reader/images/29/cccc9faf6d0addea8e1bf307cd7d8d7d.png" alt="HTTP/1.0 的短连接"></p>
...
<p><img src="/markdown-reader/images/29/80b57830e15faa17631bea74054a0e1a.png" alt="HTTP/1.1 的持久连接"></p>
...
<h3 id="2-不成熟的-http-管线化">2. 不成熟的 HTTP 管线化</h3>
...
<h3 id="3-提供虚拟主机的支持">3. 提供虚拟主机的支持</h3>
...
<h3 id="4-对动态生成的内容提供了完美支持">4. 对动态生成的内容提供了完美支持</h3>
...
<h3 id="5-客户端-cookie、安全机制">5. 客户端 Cookie、安全机制</h3>
<p>除此之外，HTTP/1.1 还引入了客户端 Cookie 机制和安全机制。其中，Cookie 机制我们在<a href="/markdown-reader/guide/03">《03 | HTTP 请求流程：为什么很多站点第二次打开速度会很快？》</a>这篇文章中介绍过了，而安全机制我们会在后面的安全模块中再做介绍，这里就不赘述了。</p>
<h2 id="总结">总结</h2>
...
`,path:"/guide/29"};export{T as default};
