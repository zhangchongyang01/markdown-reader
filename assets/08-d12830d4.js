const n={id:"08",title:"08 | 调用栈：为什么JavaScript代码会出现栈溢出？",content:`<h1 id="08--调用栈：为什么javascript代码会出现栈溢出？">08 | 调用栈：为什么JavaScript代码会出现栈溢出？</h1>
<p>在<a href="/markdown-reader/guide/07">上篇文章</a>中，我们讲到了，当一段代码被执行时，JavaScript 引擎先会对其进行编译，并创建执行上下文。但是并没有明确说明到底什么样的代码才算符合规范。</p>
<p>那么接下来我们就来明确下，哪些情况下代码才算是“一段”代码，才会在执行之前就进行编译并创建执行上下文。一般说来，有这么三种情况：</p>
<ol>
<li><p>当 JavaScript 执行全局代码的时候，会编译全局代码并创建全局执行上下文，而且在整个页面的生存周期内，全局执行上下文只有一份。</p>
</li>
<li><p>当调用一个函数的时候，函数体内的代码会被编译，并创建函数执行上下文，一般情况下，函数执行结束之后，创建的函数执行上下文会被销毁。</p>
</li>
<li><p>当使用 eval 函数的时候，eval 的代码也会被编译，并创建执行上下文。</p>
</li>
</ol>
<p>好了，又进一步理解了执行上下文，那本节我们就在这基础之上继续深入，一起聊聊<strong>调用栈</strong>。学习调用栈至少有以下三点好处：</p>
<ol>
<li><p>可以帮助你了解 JavaScript 引擎背后的工作原理；</p>
</li>
<li><p>让你有调试 JavaScript 代码的能力；</p>
</li>
<li><p>帮助你搞定面试，因为面试过程中，调用栈也是出境率非常高的题目。</p>
</li>
</ol>
<p>比如你在写 JavaScript 代码的时候，有时候可能会遇到栈溢出的错误，如下图所示：</p>
<p><img src="/markdown-reader/images/08/0c9e2c4f7ee8ca59cfa99a6f51510470.png" alt="img"></p>
...
<p>这段代码很简单，先是创建了一个 add 函数，接着在代码的最下面又调用了该函数。</p>
<p>那么下面我们就利用这段简单的代码来解释下函数调用的过程。</p>
<p>在执行到函数 add() 之前，JavaScript 引擎会为上面这段代码创建全局执行上下文，包含了声明的函数和变量，你可以参考下图：</p>
<p><img src="/markdown-reader/images/08/7fa2ed18e702861890d767ea547533da.png" alt="img"></p>
<p>从图中可以看出，代码中全局变量和函数都保存在全局上下文的变量环境中。</p>
<p>执行上下文准备好之后，便开始执行全局代码，当执行到 add 这儿时，JavaScript 判断这是一个函数调用，那么将执行以下操作：</p>
<ul>
<li><p>首先，从<strong>全局执行上下文</strong>中，取出 add 函数代码。</p>
</li>
<li><p>其次，对 add 函数的这段代码进行编译，并创建<strong>该函数的执行上下文</strong>和<strong>可执行代码</strong>。</p>
</li>
<li><p>最后，执行代码，输出结果。</p>
</li>
</ul>
<p>完整流程你可以参考下图：</p>
<p><img src="/markdown-reader/images/08/537efd9e96771dc50737117e615533ca.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/08/5e2bb65019053abfd5e7710e41d1b405.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/08/a5d7ec1f8f296412acc045835b85431d.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/08/1d50269dbc5b4c69f83662ecdd977b1d.png" alt="img"></p>
<p>接下来，<strong>第二步是调用 addAll 函数</strong>。当调用该函数时，JavaScript 引擎会编译该函数，并为其创建一个执行上下文，最后还将该函数的执行上下文压入栈中，如下图所示：</p>
<p><img src="/markdown-reader/images/08/7d6c4c45db4ef9b900678092e6c53652.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/08/ccfe41d906040031a7df1e4f1bce5837.png" alt="img"></p>
<p>当 add 函数返回时，该函数的执行上下文就会从栈顶弹出，并将 result 的值设置为 add 函数的返回值，也就是 9。如下图所示：</p>
<p><img src="/markdown-reader/images/08/03ca801a5372f941bf17d6088fee0f96.png" alt="img"></p>
<p>紧接着 addAll 执行最后一个相加操作后并返回，addAll 的执行上下文也会从栈顶部弹出，此时调用栈中就只剩下全局上下文了。最终如下图所示：</p>
<p><img src="/markdown-reader/images/08/d0ac1d6e77735338fa97cc9a3f6c717b.png" alt="img"></p>
...
<h2 id="在开发中，如何利用好调用栈">在开发中，如何利用好调用栈</h2>
<p>鉴于调用栈的重要性和实用性，那么接下来我们就一起来看看在实际工作中，应该如何查看和利用好调用栈。</p>
<h3 id="1-如何利用浏览器查看调用栈的信息">1. 如何利用浏览器查看调用栈的信息</h3>
...
<p><img src="/markdown-reader/images/08/c0d303a289a535b87a6c445ba7f34fa2.png" alt="img"></p>
...
<p><img src="/markdown-reader/images/08/abfba06cd23a7704a6eb148cff443ece.png" alt="img"></p>
<h3 id="2-栈溢出（stack-overflow）">2. 栈溢出（Stack Overflow）</h3>
...
<pre><code class="language-js">function division(a,b){
  return division(a,b)
}
console.log(division(1,2))
</code></pre>
<p>当执行时，就会抛出栈溢出错误，如下图：</p>
<p><img src="/markdown-reader/images/08/b4f7196077d9ef4eac1ca6a279f2054d.png" alt="img"></p>
...
<ul>
<li><p>每调用一个函数，JavaScript 引擎会为其创建执行上下文，并把该执行上下文压入调用栈，然后 JavaScript 引擎开始执行函数代码。</p>
</li>
<li><p>如果在一个函数 A 中调用了另外一个函数 B，那么 JavaScript 引擎会为 B 函数创建执行上下文，并将 B 函数的执行上下文压入栈顶。</p>
</li>
<li><p>当前函数执行完毕后，JavaScript 引擎会将该函数的执行上下文弹出栈。</p>
</li>
<li><p>当分配的调用栈空间被占满时，会引发“堆栈溢出”问题。</p>
</li>
</ul>
...
<p>这是一段递归代码，可以通过传入参数 n，让代码递归执行 n 次，也就意味着调用栈的深度能达到 n，当输入一个较大的数时，比如 50000，就会出现栈溢出的问题，那么你能优化下这段代码，以解决栈溢出的问题吗？</p>
<p>欢迎在留言区与我分享你的想法，也欢迎你在留言区记录你的思考过程。感谢阅读，如果你觉得这篇文章对你有帮助的话，也欢迎把它分享给更多的朋友。</p>
`,path:"/guide/08"};export{n as default};
