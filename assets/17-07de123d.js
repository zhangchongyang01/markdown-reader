const e={id:"17",title:"17 | WebAPI：XMLHttpRequest是怎么实现的？",content:`<h1 id="17--webapi：xmlhttprequest是怎么实现的？">17 | WebAPI：XMLHttpRequest是怎么实现的？</h1>
<p>在<a href="/markdown-reader/guide/16">上一篇文章</a>中我们介绍了 setTimeout 是如何结合渲染进程的循环系统工作的，那本篇文章我们就继续介绍另外一种类型的 WebAPI——XMLHttpRequest。</p>
...
<h2 id="回调函数-vs-系统调用栈">回调函数 VS 系统调用栈</h2>
...
<p>我们还是先来回顾下页面的事件循环系统，通过<a href="/markdown-reader/guide/15">《15 | 消息队列和事件循环：页面是怎么“活”起来的？》</a>的学习，你应该已经知道浏览器页面是通过事件循环机制来驱动的，每个渲染进程都有一个消息队列，页面主线程按照顺序来执行消息队列中的事件，如执行 JavaScript 事件、解析 DOM 事件、计算布局事件、用户输入事件等等，如果页面有新的事件产生，那新的事件将会追加到事件队列的尾部。所以可以说是<strong>消息队列和主线程循环机制保证了页面有条不紊地运行</strong>。</p>
...
<p><img src="/markdown-reader/images/17/d3d66afb1a103103e5c3f86c823efb77.png" alt="消息循环系统调用栈记录"></p>
...
<ul>
<li><p>第一种是把异步函数做成一个任务，添加到信息队列尾部；</p>
</li>
<li><p>第二种是把异步函数添加到微任务队列中，这样就可以在当前任务的末尾处执行微任务了。</p>
</li>
</ul>
<h2 id="xmlhttprequest-运作机制">XMLHttpRequest 运作机制</h2>
<p>理解了什么是同步回调和异步回调，接下来我们就来分析 XMLHttpRequest 背后的实现机制，具体工作过程你可以参考下图：</p>
<p><img src="/markdown-reader/images/17/2914a052f4f249a52077692a22ee5cc6.png" alt="XMLHttpRequest 工作流程图"></p>
...
<ul>
<li><p>ontimeout，用来监控超时请求，如果后台请求超时了，该函数会被调用；</p>
</li>
<li><p>onerror，用来监控出错信息，如果后台请求出错了，该函数会被调用；</p>
</li>
<li><p>onreadystatechange，用来监控后台请求过程中的状态，比如可以监控到 HTTP 头加载完成的消息、HTTP 响应体消息以及数据加载完成的消息等。</p>
</li>
</ul>
...
<p><img src="/markdown-reader/images/17/856d1965676fafa46122e3ad1235dfbf.png" alt="返回类型描述"></p>
...
<ul>
<li><p>如果网络请求出错了，就会执行 xhr.onerror；</p>
</li>
<li><p>如果超时了，就会执行 xhr.ontimeout；</p>
</li>
<li><p>如果是正常的数据接收，就会执行 onreadystatechange 来反馈相应的状态。</p>
</li>
</ul>
<p>这就是一个完整的 XMLHttpRequest 请求流程，如果你感兴趣，可以参考下 Chromium 对 XMLHttpRequest 的实现，<a href="https://chromium.googlesource.com/chromium/src/+/refs/heads/master/third_party/blink/renderer/core/xmlhttprequest/">点击这里查看代码</a>。</p>
<h2 id="xmlhttprequest-使用过程中的坑">XMLHttpRequest 使用过程中的“坑”</h2>
...
<h3 id="1-跨域问题">1. 跨域问题</h3>
<p>比如在极客邦的官网使用 XMLHttpRequest 请求极客时间的页面内容，由于极客邦的官网是<a href="http://www.geekbang.org%EF%BC%8C%E6%9E%81%E5%AE%A2%E6%97%B6%E9%97%B4%E7%9A%84%E5%AE%98%E7%BD%91%E6%98%AFtime.geekbang.org%EF%BC%8C%E5%AE%83%E4%BB%AC%E4%B8%8D%E6%98%AF%E5%90%8C%E4%B8%80%E4%B8%AA%E6%BA%90%EF%BC%8C%E6%89%80%E4%BB%A5%E5%B0%B1%E6%B6%89%E5%8F%8A%E5%88%B0%E4%BA%86%E8%B7%A8%E5%9F%9F%EF%BC%88%E5%9C%A8">www.geekbang.org，极客时间的官网是time.geekbang.org，它们不是同一个源，所以就涉及到了跨域（在</a> A 站点中去访问不同源的 B 站点的内容）。默认情况下，跨域请求是不被允许的，你可以看下面的示例代码：</p>
...
<p>你可以在控制台测试下。首先通过浏览器打开<a href="http://www.geekbang.org%EF%BC%8C%E7%84%B6%E5%90%8E%E6%89%93%E5%BC%80%E6%8E%A7%E5%88%B6%E5%8F%B0%EF%BC%8C%E5%9C%A8%E6%8E%A7%E5%88%B6%E5%8F%B0%E8%BE%93%E5%85%A5%E4%BB%A5%E4%B8%8A%E7%A4%BA%E4%BE%8B%E4%BB%A3%E7%A0%81%EF%BC%8C%E5%86%8D%E6%89%A7%E8%A1%8C%EF%BC%8C%E4%BC%9A%E7%9C%8B%E5%88%B0%E8%AF%B7%E6%B1%82%E8%A2%AB">www.geekbang.org，然后打开控制台，在控制台输入以上示例代码，再执行，会看到请求被</a> Block 了。控制台的提示信息如下：</p>
<pre><code>Access to XMLHttpRequest at &#39;https://time.geekbang.org/&#39; from origin &#39;https://www.geekbang.org&#39; has been blocked by CORS policy: No &#39;Access-Control-Allow-Origin&#39; header is present on the requested resource.
</code></pre>
<p>因为 <a href="http://www.geekbang.org">www.geekbang.org</a> 和 time.geekbang.com 不属于一个域，所以以上访问就属于跨域访问了，这次访问失败就是由于跨域问题导致的。</p>
<h3 id="2-https-混合内容的问题">2. HTTPS 混合内容的问题</h3>
<p>了解完跨域问题后，我们再来看看 HTTPS 的混合内容。HTTPS 混合内容是 HTTPS 页面中包含了不符合 HTTPS 安全要求的内容，比如包含了 HTTP 资源，通过 HTTP 加载的图像、视频、样式表、脚本等，都属于混合内容。</p>
<p>通常，如果 HTTPS 请求页面中使用混合内容，浏览器会针对 HTTPS 混合内容显示警告，用来向用户表明此 HTTPS 页面包含不安全的资源。比如打开站点 <a href="https://www.iteye.com/groups">https://www.iteye.com/groups</a> ，可以通过控制台看到混合内容的警告，参考下图：</p>
<p><img src="/markdown-reader/images/17/4b4a210a1e078d9a26fe31e6eab34963.png" alt="HTTPS 混合内容警告"></p>
<p>从上图可以看出，通过 HTML 文件加载的混合资源，虽然给出警告，但大部分类型还是能加载的。而使用 XMLHttpRequest 请求时，浏览器认为这种请求可能是攻击者发起的，会阻止此类危险的请求。比如我通过浏览器打开地址 <a href="https://www.iteye.com/groups">https://www.iteye.com/groups</a> ，然后通过控制台，使用 XMLHttpRequest 来请求 <a href="http://img-ads.csdn.net/2018/201811150919211586.jpg">http://img-ads.csdn.net/2018/201811150919211586.jpg</a> ，这时候请求就会报错，出错信息如下图所示：</p>
<p><img src="/markdown-reader/images/17/46c22d4e54815942c1a86f11b14516a1.png" alt="使用 XMLHttpRequest 混合资源失效"></p>
<h2 id="总结">总结</h2>
...
<p>对比<a href="/markdown-reader/guide/16">上一篇文章</a>，setTimeout 是直接将延迟任务添加到延迟队列中，而 XMLHttpRequest 发起请求，是由浏览器的其他进程或者线程去执行，然后再将执行结果利用 IPC 的方式通知渲染进程，之后渲染进程再将对应的消息添加到消息队列中。如果你搞懂了 setTimeout 和 XMLHttpRequest 的工作机制后，再来理解其他 WebAPI 就会轻松很多了，因为大部分 WebAPI 的工作逻辑都是类似的。</p>
<h2 id="思考时间">思考时间</h2>
...
`,path:"/guide/17"};export{e as default};
